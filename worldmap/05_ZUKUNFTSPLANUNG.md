...
| 1     | 1.9  | ✅ Umgesetzt                                                    | [Applikationsweites Error-Tracking & Alerting](../docs/architecture/05_1.9_ERROR_TRACKING_SENTRY.md)                                                                   | 18*                  | 35              | 15             | 50             | 100               | Hoch          | DevOps-Slayer + Security-Auditor   | Ja (Log-Inhalt)       | Nein                   | Nein                       | Niedrig        | Verändert Bestand                       | keine                                                  |
...
| 2     | 2.2  | ✅ Umgesetzt                                                    | Telegram-Benachrichtigungen (Pivot von Web-Push, [Detail](../docs/architecture/05_2.2_TELEGRAM_NOTIFICATIONS.md))                                                      | 7                    | 42              | 25             | 50             | 75                | Hoch          | Growth-Hacker                      | Ja                    | Nein                   | Ja                         | Niedrig        | Additiv                                 | keine                                                  |
...
#### 1.9 — Applikationsweites Error-Tracking & Alerting — **Executed** 2026-08-12

- **Detail-Execution-Plan:** [`docs/architecture/05_1.9_ERROR_TRACKING_SENTRY.md`](../docs/architecture/05_1.9_ERROR_TRACKING_SENTRY.md) — vollständiger Implementationsplan + Execution-Ergebnis (Abschnitt 1.1), archiviert nach Ausführung analog `05_1.7_USER_STATS_ANALYTICS.md`.
- **Ergebnis:** Sentry (SaaS, EU-Region) für app-weites Error-Tracking. Zentraler `beforeSend`-Redaction-Filter (`src/lib/casino/sentry-scrub.ts`, TDD) entfernt Cookies/Auth-Header und redigiert `serverSeed`/bekannte Secret-Muster, unterscheidet dabei `serverSeed` korrekt von `serverSeedHash`. `CasinoLogger.error()` und `enforceRateLimit()`s Fail-Closed-Pfad senden automatisch an Sentry — deckt alle bestehenden Aufrufstellen ohne Einzeldatei-Wiring ab. Live in Sentry verifiziert (`Insufficient balance`, `POST /api/casino/bet`, korrekt getaggt, ohne Secrets).
- **Next.js-16-Fund:** `src/proxy.ts` läuft standardmäßig auf der Node.js-Runtime statt Edge (Breaking Change) — direkt aus den lokalen Next-Docs verifiziert statt angenommen.
- **Security-Review:** 2 Befunde (zu breite CSP-Wildcard, fehlendes try/catch in den Error-Boundaries) gefunden und behoben.
- **Nebenbefund:** vorbestehender, wiederkehrender `getUserSeeds RPC failed`-Fehler (`WalletService`) durch die neue Instrumentierung erstmals sichtbar geworden — außerhalb dieses Scopes, noch offen.
- **Verifizierung:** `npx tsc --noEmit` 0 Fehler, `npx eslint .` 0 neue Fehler, `npx vitest run` 430/430 grün, `npm run vibe-check` grün, `npm run build` grün.
...
- **Status:** umgesetzt, live und per echtem End-to-End-Test bestätigt. Detail, Implementationsplan und Execution-Ergebnis: [docs/architecture/05_2.2_TELEGRAM_NOTIFICATIONS.md](../docs/architecture/05_2.2_TELEGRAM_NOTIFICATIONS.md).
...
- **Abhängigkeit:** keine.
- **Verifizierung:** `npx tsc --noEmit` 0 Fehler, `npx vitest run` 415/415 grün, gezieltes ESLint 0 Fehler, `npm run vibe-check` und `npm run build` grün, `security-reviewer`-Agent-Durchlauf ohne CRITICAL/HIGH (2 LOW sofort behoben). Migration `025` live, alle Secrets in `.env.local` und Vercel gesetzt, Webhook bei Telegram registriert. Echter End-to-End-Test mit Jans Account (`film05`, 2026-08-12): Linking und ein echter Big Win ($600 Dice-Bet @ 2,00x → $1.200 Payout) mit realer Telegram-Zustellung bestätigt. `/stop`-Test und Purge-Executor per Jan-Entscheidung nicht zusätzlich manuell nachgetestet — Details siehe Datei Abschnitt 9.

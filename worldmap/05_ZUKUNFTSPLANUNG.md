...
**Jans aktive Themen (Stand 2026-08-14):** Die Jan-Tabelle zeigt ausschließlich offene, in Arbeit befindliche oder noch nicht abschließend verifizierte Initiativen. Die zehn abgeschlossenen Punkte `1.1`, `1.2`, `1.4`, `1.5`, `1.6`, `1.7`, `1.9`, `2.2`, `2.5` und `2.7` sowie der bereits abgeschlossene `2.4` stehen im [Archivindex](../docs/archive/05_ZUKUNFTSPLANUNG_ARCHIV.md). `2.7` wird dort als technisch produktiv abgeschlossen geführt; die zeitbedingte Kostenplausibilisierung bleibt als Archivhinweis sichtbar. `1.11` bleibt ausschließlich als lokale Ausschlussnotiz im [Security-Archiv](../docs/architecture/06_SECURITY_CASINO_LOCAL_CLOSURE.md).

---

**Punkt-ID:** Die fortlaufende Kurznummer (`P01`–`P24`) bleibt über Archiv und aktive Roadmap stabil. Sie dient nur der Bezugnahme; **Phase/Nr.** bleibt die fachliche Roadmap-Klassifikation. `offen/TBD` bedeutet, dass noch keine Phase festgelegt ist.
...
| P17   |     2 |  2.8 | ⬜ Nicht gestartet | Anti-Fraud                   | Read-only-Signale und Admin-Review planen            |      70 |     20 |     55 | Sehr Hoch  | Ja           | Nein       | Additiv           |
| P19   |     2 |  2.9 | ⬜ Nicht gestartet | PostHog-Analytics            | Consent-Gate und Event-Allowlist festlegen           |      48 |     30 |     64 | Sehr Hoch  | Nein         | Nein       | Additiv           |
| P20   |     1 | 1.13 | ⬜ Nicht gestartet | Uptime-Kuma-Monitoring       | Staging-Healthcheck und Incident-Lauf durchführen    |      42 |     22 |     58 | Hoch       | Nein         | Nein       | Additiv           |
...
**Empfohlene Reihenfolge:** Zuerst 1.10 abschließen, dann die Messbasis von 2.7 aus dem Archivhinweis dokumentieren und anschließend 2.6 konkretisieren. Danach folgen 2.8 sowie die Lernstrecke 2.9, 1.13, 1.14 und 2.10. Phase 4 (4.1 Outbox, 4.2 Trigger.dev) und Phase 3 bleiben nachgelagert.
...
- **Status:** Nicht gestartet. PostHog wird als Lernsystem für Product-Analytics geplant, nicht als Ersatz für Sentry und nicht als Wallet-/Casino-Autorität.
...
- **Scope:** Consent-gesteuerte Produkt-Events. Minimale Allowlist (`landing_viewed`, `cta_play_now_clicked`, `sign_up_completed`, `first_game_started`, `stats_viewed`); kein Wallet-Saldo, kein Bet-Betrag, keine E-Mail und keine Session-Replays in v1.
- **Neue DB-Objekte:** keine geplant — PostHog speichert im eigenen externen Event-Store. Self-Hosting würde separate Infrastruktur/Datenspeicher benötigen und wird nicht in die produktive Casino-Supabase-Datenbank eingebaut; First-Party-Telemetrie bleibt eine separate Initiative.
- **Abhängigkeit:** Jans Entscheidung zu Consent/Privacy; keine Abhängigkeit vom Money-Pfad.
- **Verifizierung:** Jeder erlaubte Test-Event landet im PostHog-Projekt; Opt-out verhindert Events; Negativtests bestätigen, dass Wallet-/PII-Felder nicht im Payload landen.
- **Security-Reviewer:** Pflicht (externe API, Consent, mögliche PII und Identitätsverknüpfung).

#### 1.13 — Uptime-Kuma-Production-Monitoring

- **Status:** Nicht gestartet. Uptime Kuma ist die Open-Source-Lernstrecke ohne zusätzliche SaaS-Lizenz; Server-/Hostingkosten und Betriebsaufwand bleiben möglich.
- **Ziel:** Einen kontrollierten Betriebsablauf lernen: Ausfall erkennen, Alarm erhalten, Ursache eingrenzen, Dienst wiederherstellen und den Vorfall dokumentieren.
- **Scope:** Eine öffentliche Liveness-Route `src/app/api/health/route.ts` ohne Datenbank-, Wallet- oder Secret-Inhalt; separate Uptime-Kuma-Instanz für Production-/Staging-URL und Health-Route; Benachrichtigungskanal konfigurieren; ein dokumentierter Incident-Test mit absichtlich nicht erreichbarer Staging-Route; Recovery- und Post-Mortem-Checkliste.
- **Neue DB-Objekte:** keine im Casino-Projekt. Uptime Kuma speichert Monitor-/Incident-Daten in seiner eigenen Instanz; die App erhält keine Incident-Schreiblogik und keine neue Supabase-Migration.
- **Abhängigkeit:** stabile Production-/Staging-URL und erreichbare Uptime-Kuma-Instanz; 1.9 Sentry ist empfohlen, damit App-Fehler und externe Verfügbarkeit getrennt betrachtet werden können.
- **Verifizierung:** Liveness liefert im Normalzustand `200` ohne interne Daten; ein Staging-Fehler erzeugt genau einen nachvollziehbaren Incident; Recovery schließt den Incident; Production wird während des Tests nicht absichtlich gestört.
- **Security-Reviewer:** Pflicht für die öffentliche Health-Route (keine internen Statuswerte, keine Stack-Traces, keine Supabase-/Upstash-Geheimnisse).

#### 1.14 — Upstash-Vertiefung

- **Status:** Teilweise vorhanden. Redis und Rate Limiting sind bereits im Projekt vorhanden; diese Initiative vertieft Verständnis, Messbarkeit und eine sichere nicht-monetäre Nutzung.
- **Ziel:** Verstehen, wann Redis gegenüber Supabase sinnvoll ist: schnelle, kurzlebige Daten für Rate Limits und Cache, aber keine autoritativen Geldwerte.
- **Scope:** bestehendes Rate Limiting dokumentieren und mit Upstash-Analytics auswerten; einen ungefährlichen Cache für öffentliche Spielkonfiguration oder autorisierte Guide-Read-Daten ergänzen; TTL, Cache-Miss, Cache-Hit, Stale-Daten und Upstash-Ausfall testen; kein Wallet-, Bet-, Payout- oder Settlement-Wert wird aus dem Cache gelesen.
- **Neue DB-Objekte:** keine. Die bestehende Upstash-ENV und das vorhandene Rate-Limit-Modul werden verwendet; eine neue Redis-Datenbank wird für den ersten Lernschritt nicht vorausgesetzt.
- **Abhängigkeit:** bestehende `UPSTASH_REDIS_REST_URL`-/`UPSTASH_REDIS_REST_TOKEN`-Konfiguration; 1.10 Chaos-Testing ist als Verifikationsvorlage empfohlen.
- **Verifizierung:** Cache-Hit und Cache-Miss liefern identische fachliche Daten; Ablauf nach TTL funktioniert; ein Upstash-Ausfall führt zum definierten sicheren Verhalten; Tests beweisen, dass der Wallet-Pfad den Cache nicht verwendet.
- **Security-Reviewer:** Pflicht (Rate Limiting ist eine API-Sicherheitsgrenze; Cache-Leakage und falsche Autorität müssen ausgeschlossen werden).
...
| R13 | PostHog-Events enthalten versehentlich Walletwerte/PII oder werden doppelt bzw. trotz Opt-out gesendet                                     | 2.9                        | Mittel                | Hoch       | Kleine Event-Allowlist; Consent-Gate vor PostHog; Payload-Tests ohne E-Mail/Saldo/Bet-Betrag; v1 ohne Session Replay                                                                                                                                                                                                                                                                                                                                                                       |
...
| R15 | Upstash-Cache liefert veraltete Daten oder wird versehentlich als Wallet-Autorität verwendet                                               | 1.14                       | Niedrig               | Hoch       | Cache ausschließlich für nicht-monetäre Daten; TTL-/Ausfalltests; statische Code-Grenze: Wallet-Snapshot nur aus bestehendem serverautoritativen Pfad                                                                                                                                                                                                                                                                                                                                      |
...
- **Traceability erhalten:** Punkt-IDs P01–P24 bleiben stabil; archivierte Detaildateien werden nicht gelöscht.
- **Aktiver Plan vollständig:** 1.10, 2.6, 2.8, Outbox, 2.9, 1.13, 1.14, Trigger.dev, 2.10, 2.11 und 3.1–3.3 erscheinen in Tabelle und/oder aktivem Abschnitt 3.

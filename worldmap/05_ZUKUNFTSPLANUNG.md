...
| 1     | 1.10 | ⬜ Nicht gestartet                     | Resilience-/Chaos-Testing                                                                                                                       | 19*                  | 38              | 25             | 65             | 100               | Hoch          | Security-Auditor + Bug-Hunter      | Nein                  | Ja                     | Nein                       | Niedrig        | Additiv                                 | 1.9 empfohlen, nicht zwingend                          |
...
- **Scope:** neues Skript-Verzeichnis `scripts/chaos/`, gezielte Fehler-Injektion (z. B. Supabase-URL temporär ungültig setzen, Upstash-Credentials invalidieren) **ausschließlich gegen eine isolierte Dev-/Test-Instanz**, Beobachtung von `src/app/api/casino/bet/route.ts` und `src/proxy.ts`.
- **Neue DB-Objekte:** keine.
- **Abhängigkeit:** 1.9 empfohlen (Fehlerverhalten während der Tests sichtbar), nicht zwingend.
- **Verifizierung:** Bet-Route antwortet bei simuliertem DB-Ausfall mit 503 statt stillem Fehler/falschem 200; Rate-Limiter fällt bei Upstash-Ausfall auf den dokumentierten In-Memory-Fallback zurück.
- **Security-Reviewer:** Nein (reine Verifikation bestehenden Verhaltens gegen isolierte Testumgebung, kein neuer Code-Pfad in Produktion) — siehe aber R9 im Risiko-Register.
...
| R9  | Chaos-Tests laufen versehentlich gegen echte Produktionsdaten/-nutzer statt Dev/Staging (Red-Teaming-Äquivalent: siehe `06-security-casino.md` §5) | 1.10                       | Mittel             | Hoch       | Zwingend nur gegen isolierte Dev-/Test-Supabase-Instanz mit synthetischen Test-Accounts ausführen, niemals gegen Live-DB; expliziter Umgebungs-Check vor jedem Testlauf                  |

...
**Backlog-Gate geprüft:** Laut AGENTS.md „Backlog First" haben offene `01_WORLDMAP_STATUS.md`-Punkte Vorrang vor neuen Features. Aktuell offen: `02_FRONTEND_REDESIGN.md` (In Execution), `04_docs_ordnung.md` (Execution-Ready), `01-offene-commits.md` (Geplant), Seed-Siegel-Verfahren (in `01_WORLDMAP_STATUS.md` §2 Punkt 2 bereits als Zukunftsoption vermerkt). Keine Kategorie liegt unter Top 50 %. **Kein Backlog-Konflikt** — diese Roadmap ergänzt, verdrängt nichts. Initiative 1.2 (Seed-Siegel-Verfahren) ist die Formalisierung des bereits in `01_WORLDMAP_STATUS.md` §2 notierten Punkts, keine Dopplung.
...
### 1.0 — Jans ausgewählte Langzeit-Themen (Stand 2026-08-10)

Aus dem Gesamtkatalog unten von Jan explizit für die Weiterverfolgung ausgewählt — reine Übersicht/Merkliste, keine Statusänderung an den zugehörigen Zeilen der Haupttabelle.

| Nr  | Thema                           | Verankerung in dieser Datei                        | ROI-Rang (bestehend) | Neues Skillfeld (Kern-Lerneffekt)                                             |
| --- | ------------------------------- | -------------------------------------------------- | -------------------- | ----------------------------------------------------------------------------- |
| 2.4 | LLM-Chat-Bot-Erweiterung        | Abschnitt 3, Phase 2 (Zeile ~157–164)              | 16                   | AI-Integration, Prompt-Engineering, Prompt-Injection-Abwehr                   |
| 1.2 | Seed-Siegel-Verfahren           | Abschnitt 3, Phase 1 (Zeile ~70–78)                | 8                    | Kryptografisches Seed-Siegel-Protokoll (Hash-Chain statt Seed-pro-Bet)        |
| —   | CI/CD-Pipeline (GitHub Actions) | **fehlt** — keine eigene Initiative in Abschnitt 3 | —                    | Automatisierte Build/Test/Lint-Gates remote statt nur lokalem Pre-Commit-Hook |

**Offen:** CI/CD-Pipeline hat noch keine Initiativen-Nummer und keinen Abschnitt-3-Eintrag (Ziel/Scope/DB-Objekte/Verifizierung fehlen). Soll ich das als reguläre Initiative `1.9` mit vollständigem Abschnitt-3-Eintrag ergänzen, oder bleibt es vorerst nur in dieser Merkliste?
...
| 1     | 1.2 | ⬜ Nicht gestartet                     | Seed-Siegel-Verfahren                                                                                       | 8                    | 45              | 25             | 50             | 71                | Hoch       | Logic-Architect                    | Ja                | Ja (RNG-Pfad) | Nein                            | Mittel         | Verändert Bestand                       | keine                               |
...
| 1     | 1.7 | ⬜ Nicht gestartet                     | Eigene User-Stats-Analytics-Seite                                                                           | 4                    | 25              | 12             | 35             | 95                | Niedrig    | Design-Guardian                    | Nein              | Nein          | Nein                            | Niedrig        | Additiv                                 | keine                               |
| 1     | 1.8 | ⬜ Nicht gestartet                     | Internationalisierung (EN/DE)                                                                               | 10                   | 55              | 22             | 50             | 65                | Mittel     | DevOps-Slayer                      | Nein              | Nein          | Optional (`users.locale`)       | Mittel         | Verändert Bestand                       | keine                               |
| 2     | 2.1 | ⬜ Nicht gestartet                     | Neues Spiel (Plinko/Mines)                                                                                  | 6                    | 50              | 35             | 65             | 76                | Niedrig    | Logic-Architect + UI-Animator      | Ja                | Ja            | Nein                            | Niedrig        | Verändert Bestand (Routing)             | keine                               |
| 2     | 2.2 | ⬜ Nicht gestartet                     | Web-Push-Benachrichtigungen                                                                                 | 7                    | 42              | 25             | 50             | 75                | Hoch       | Growth-Hacker                      | Nein              | Nein          | Ja                              | Niedrig        | Additiv                                 | keine                               |
| 2     | 2.3 | ⬜ Nicht gestartet                     | Referral-/Freundschaftssystem                                                                               | 13                   | 52              | 50             | 55             | 54                | Mittel     | Growth-Hacker                      | Ja                | Ja            | Ja                              | Mittel         | Additiv                                 | keine                               |
| 2     | 2.4 | ⬜ Nicht gestartet                     | Chat-Bot-Erweiterung (LLM-gestützt)                                                                         | 16                   | 40              | 30             | 35             | 50                | Hoch       | Growth-Hacker                      | Ja (externe API)  | Nein          | Nein                            | Niedrig        | Verändert Bestand                       | keine                               |
| 2     | 2.5 | ⬜ Nicht gestartet                     | Admin-BI-Dashboard (Cohort/Retention)                                                                       | 9                    | 45              | 22             | 45             | 67                | Hoch       | DevOps-Slayer                      | Nein              | Nein          | Optional (materialisierte View) | Niedrig        | Additiv                                 | keine                               |
...
- **Scope:** eigener, minimaler Service Worker nur für Push-Registrierung (kein volles PWA-/Offline-Setup nötig, da 1.3 verworfen), neue `src/app/api/push/subscribe/route.ts`, `src/lib/casino/push-manager.ts` (neu).
- **Neue DB-Objekte:** Tabelle `push_subscriptions` (user_id, endpoint, keys).
- **Abhängigkeit:** keine eigene Initiative mehr — Service Worker wird innerhalb von 2.2 selbst aufgesetzt.

...
**Jans aktive Themen (Stand 2026-08-15):** Die Jan-Tabelle zeigt ausschließlich offene, in Arbeit befindliche oder noch nicht abschließend verifizierte Initiativen. Die elf abgeschlossenen Punkte `1.1`, `1.2`, `1.4`, `1.5`, `1.6`, `1.7`, `1.9`, `1.14`, `2.2`, `2.5` und `2.7` sowie der bereits abgeschlossene `2.4` stehen im [Archivindex](../docs/archive/05_ZUKUNFTSPLANUNG_ARCHIV.md). `2.7` wird dort als technisch produktiv abgeschlossen geführt; die zeitbedingte Kostenplausibilisierung bleibt als Archivhinweis sichtbar. `1.11` bleibt ausschließlich als lokale Ausschlussnotiz im [Security-Archiv](../docs/architecture/06_SECURITY_CASINO_LOCAL_CLOSURE.md).
...
| P17   |     2 |  2.8 | ✅ Live            | Anti-Fraud                   | XFF-Vertrauen der Deployment-Plattform verifizieren (R22) |      75 |     25 |     55 | Sehr Hoch  | Ja           | Nein       | Additiv           |
| P19   |     2 |  2.9 | 🔄 In Execution    | PostHog-Analytics            | Jan: Go-Live-Gates (§9b) + Personal API Key (M11)         |      48 |     30 |     64 | Sehr Hoch  | Nein         | Nein       | Additiv           |
| P20   |     1 | 1.13 | 🔄 In Execution    | Production-Monitoring        | Externen Free-Checker einrichten (05_1.13)                |      42 |     22 |     58 | Hoch       | Nein         | Nein       | Additiv           |
...
**Empfohlene Reihenfolge:** Zuerst 1.10 abschließen, dann die Messbasis von 2.7 aus dem Archivhinweis dokumentieren und anschließend 2.6 konkretisieren. Danach folgen 2.8 sowie die Lernstrecke 2.9, 1.13 und 2.10. Phase 4 (4.1 Outbox, 4.2 Trigger.dev) und Phase 3 bleiben nachgelagert.
...
- **Status:** Vollständig live (2026-08-16) — Detailplan: [`worldmap/05_2.8_Anti_Fraud.md`](05_2.8_Anti_Fraud.md), Abschnitt 11.6. **Scope-Korrektur ggü. dem ursprünglichen Entwurf unten (2026-08-15):** Es existierte bereits eine passende Signal-Infrastruktur aus einer früheren Initiative (`risk_events`-Tabelle, Migration `029`) — die geplante eigene `fraud_flags`-Tabelle hätte diese dupliziert. Umgesetzt wurde stattdessen eine Erweiterung von `risk_events` um zwei neue Signaltypen (`bet_velocity`, `win_rate_anomaly`) plus eine minimale, gehashte, 30-Tage-purge-begrenzte Fingerprint-Tabelle für die Multi-Account-Korrelation (`risk_events.evidence` darf laut bestehendem Redaktionsdesign nie eine IP enthalten). Plan durchlief 3 unabhängige Reviews (Architektur, Security, Datenbank) vor Execution plus Security-/Code-Review danach — mehrere kritische Design-Fehler gefunden und behoben (u. a. hätte eine reine CHECK-Constraint-Erweiterung die neuen Signale stillschweigend verworfen, da `record_risk_event()` die Werteliste zusätzlich hart codiert prüft). `tsc`/ESLint/Vitest (559 Tests) grün. Migration `030_fraud_signal_detection.sql` geschrieben und manuell geprüft, aber mangels lokalem Docker-/Supabase-Stack **nicht live angewendet** — eigener, expliziter Rollout-Schritt.
...
- **Scope (finale Fassung, siehe Detailplan Abschnitt 3.7):** `src/lib/casino/fraud-detection.ts` (Orchestrator: Bet-Frequenz-Ausreißer, IP-Cluster, Win-Rate-Abweichung ggü. Kohorte), `src/lib/casino/network-fingerprint.ts` (HMAC-IP-Hashing), `src/app/api/admin/fraud/route.ts` (GET/PATCH Review-Queue) + `.../fraud/scan/route.ts` (POST, admin-getriggerter Scan statt Cron in v1), `src/app/admin/fraud/page.tsx`.
- **Neue DB-Objekte:** `risk_events`-Erweiterung (additiv, kein neues Schema), Tabelle `bet_network_fingerprints` (gehashte IP, 30-Tage-Purge), Tabelle `fraud_scan_lock` (atomarer Row-Lock statt Session-Advisory-Lock), drei read-only SQL-Aggregations-RPCs; liest read-only aus bestehenden `wallet_transactions`, keine neue Schreiblast auf den Settlement-Pfad.
...
- **Verifizierung:** siehe Detailplan Abschnitt 7 (synthetische Signal-Tests, Fingerprint-Determinismus, Admin-Route-Auth 401/403/200/409, Money-Pfad-Negativtest) — alle grün.
- **Security-Reviewer:** Durchgeführt (liest Wallet-/Bet-Daten über User-Grenzen hinweg, Admin-Boundary, siehe R11/R18–R22 im Detailplan-Risiko-Register). Ein offener Punkt: XFF-Vertrauen der Deployment-Plattform unverifiziert (R22), deshalb Multi-Account-Signal in der Admin-UI als „beobachtend" markiert statt beweiskräftig behandelt.
...
#### 1.13 — Production-Monitoring (Health-Route + gehosteter Free-Checker)

- **Status:** In Execution, Scope am 2026-08-15 vereinfacht. Health-Route `src/app/api/health/route.ts` implementiert und gegen Production verifiziert. Ursprünglich geplantes Self-Hosting (Uptime Kuma auf Jans VPS + dedizierter Telegram-Bot) auf Jans Wunsch verworfen — kein Server-Betrieb, kein neuer Bot. Offen: Jan richtet einen kostenlosen externen Checker (z. B. UptimeRobot) auf `/api/health` ein. Details: [05_1.13_Uptime-Kuma-Monitoring.md](05_1.13_Uptime-Kuma-Monitoring.md).
- **Ziel:** Bei einem Ausfall benachrichtigt werden — ohne eigenen Betriebsaufwand.
- **Scope:** Öffentliche Liveness-Route ohne Datenbank-, Wallet- oder Secret-Inhalt; externer, kostenloser Checker-Dienst (Jans Account, E-Mail-Alarm); optionaler risikofreier Testlauf über eine dedizierte Chaos-Umgebungsvariable.
- **Neue DB-Objekte:** keine.
- **Abhängigkeit:** keine.
- **Verifizierung:** Liveness liefert im Normalzustand `200` ohne interne Daten (bereits gegen Production bestätigt); optionaler Testlauf zeigt einen Alarm und eine Recovery-Nachricht.
- **Security-Reviewer:** Bereits durchgeführt für die Health-Route (keine internen Statuswerte, keine Stack-Traces, keine Supabase-/Upstash-Geheimnisse).

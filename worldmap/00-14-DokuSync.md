...
| M6     | CI-Wiring (Pflicht-Gate + wöchentlicher Cron) |           🟡 Lokal verifiziert; echter Remote-Lauf braucht Push           | `quality-ci.yml`-Schritt + neue `doc-drift-check.yml` angelegt, Blockierverhalten lokal simuliert und bestätigt                                      |      LLM      |
...
> **Update 2026-08-29 (nach Ausführung M1–M6):** #1 (PR-Template existiert), #2 (Register auf Stand 2026-08-29), #3 (Script real & fehlerbereinigt), #4 (CI-Gate lokal verifiziert), #7 (Index-Zähler automatisiert, deckt bereits reale Drift auf), #8 (Mojibake-Check als Nebenprodukt live), #9 (ROOTS erweitert, fand real 1 neuen toten Link), #10 (Cron-Workflow angelegt) sind alle spürbar verbessert; #5 (CLAUDE.md-Pflicht technisch erzwingen) und #6 (strukturiertes PR-Review) bleiben bewusst unverändert (siehe Anhang). Neuer rechnerischer Schnitt: (20+15+10+20+85+60+15+30+15+25)/10 = **Top 29,5 %**. Die Headline-Angabe in `00-14-VersionControlDoku.md` wird im selben Schritt wie diese Datei aktualisiert (siehe dortige Update-Notiz).
...
**L3 — Lokal verifiziert, Remote-Lauf offen (🟡 teilverifiziert, 2026-08-29):** Lokale Simulation durchgeführt: Testdatei `docs/__tmp_link_test.md` mit bewusst kaputtem Link angelegt → `npm run check-doc-links` schlägt real mit Exit-Code 1 fehl und meldet exakt diesen Link (`[LIVE] docs\__tmp_link_test.md -> ../does/not/exist.md`) → Testdatei entfernt → Folgelauf zeigt wieder den bekannten Baseline-Stand (20 Live-Treffer, keine neuen). Das bestätigt, dass der `quality-ci.yml`-Schritt bei einem echten neuen toten Link real blockieren würde. **Nicht abschließend verifizierbar ohne Push:** ein tatsächlicher GitHub-Actions-Lauf (PR-Kontext für M1/L1, `workflow_dispatch`-Testlauf für den Cron-Workflow) — beides setzt einen Push nach `origin` voraus, der der globalen Commit-/Push-Freigabe unterliegt und in dieser Session nicht angefordert wurde.
...
- ✅ Rechnerischer Schnitt der 10 Sub-Unterkategorien verbessert sich von Top 85,5 % auf **Top 29,5 %** (Update 2026-08-29, siehe Abschnitt 2).
- 🔲 **Offen, außerhalb dieser Session lösbar:** echter GitHub-Actions-Lauf für M2/M6 (setzt einen Push nach `origin/main` bzw. einen realen PR voraus).

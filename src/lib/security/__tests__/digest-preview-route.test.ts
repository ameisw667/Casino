...
describe('POST /api/admin/digest-preview/start', () => {
  ...
  it('returns 503 if TRIGGER_SECRET_KEY is missing', async () => {
    mocks.getUser.mockResolvedValue({ data: { user: { id: 'u1', email: 'admin@example.com' } }, error: null });
    ...
    expect(body.error).toContain('TRIGGER_SECRET_KEY');
  });
  ...
  it('triggers preview run and generates scoped public access token on admin request', async () => {
    ...
    expect(body.runId).toBe('run_preview_123');
    expect(body.publicAccessToken).toBe('pk_test_token_abc');

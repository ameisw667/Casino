...
describe('CSP report aggregation (06_csp_violation_reporting L4)', () => {
  it('groups identical violations by directive and blocked URI with a counter', () => {
    const result = normalizeCspReports([
      { type: 'csp-violation', body: { violatedDirective: 'script-src', blockedURL: 'https://e/x' } },
      { type: 'csp-violation', body: { violatedDirective: 'script-src', blockedURL: 'https://e/x' } },
      { type: 'csp-violation', body: { violatedDirective: 'script-src', blockedURL: 'https://e/y' } },
      { type: 'csp-violation', body: { violatedDirective: 'style-src', blockedURL: 'https://e/x' } },
    ]);

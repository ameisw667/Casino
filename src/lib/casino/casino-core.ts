...
export function calculateDicePayout(
  ...
  if (bet > config.limits.maxBetHardcap) {
    throw new RangeError(
      `Bet ${bet} exceeds maximum allowed bet of ${config.limits.maxBetHardcap}`,
    );
  }
...
export class CasinoCore {
  ...
  static isRouletteWin(betType: RouletteBetType, roll: number): boolean {
    ...
    if (betType.type === 'EVEN_ODD')
      return roll !== 0 && (betType.value === 'EVEN' ? roll % 2 === 0 : roll % 2 !== 0);
    if (betType.type === 'RANGE')
      return (
        roll !== 0 &&
        (betType.value === '1-18' ? roll >= 1 && roll <= 18 : roll >= 19 && roll <= 36)
      );

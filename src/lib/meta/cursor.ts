...
const LeaderboardCursorPayloadSchema = z
  .object({
    v: z.literal(2),
  ...
  .superRefine((value, context) => {
    ...
    const inverted =
      value.periodStart !== null && Date.parse(value.periodStart) >= Date.parse(value.asOf);
    if (allTimeHasStart || boundedHasNoStart || inverted) {
      context.addIssue({
        code: 'custom',
        message: 'Invalid leaderboard snapshot',
        path: ['periodStart'],
      });
    }
  });
...
export function decodeLeaderboardCursor(
  ...
  if (
    expected &&
    (parsed.data.period !== expected.period || parsed.data.metric !== expected.metric)
  ) {
    throw new Error('Invalid cursor');
  }

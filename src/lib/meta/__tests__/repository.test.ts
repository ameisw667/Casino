...
describe('history cursors', () => {
  it('retains the UUID tie-breaker for identical timestamps', () => {
    ...
    expect(decodeHistoryCursor(cursor)).toEqual({ settledAt: '2026-08-06T12:00:00.000Z', id: ids.resultA });
  ...
  it('rejects malformed and cross-kind cursors', () => {
    const leaderboard = encodeLeaderboardCursor({
      period: 'all-time', metric: 'wagered', asOf: '2026-08-06T12:00:00.000Z', periodStart: null,
      value: '10.00', playerKey: 'a'.repeat(64),
    });
...
describe('meta repository', () => {
  ...
  it('uses a bounded history allowlist and exact page-only decimal summary', async () => {
    const client = new ClientStub({ game_results: [{ data: [
      historyRow(),
      historyRow({ id: ids.resultB, wager_amount: '0.20', payout_amount: '0.10', net_amount: '-0.10', outcome: 'loss' }),
    ], error: null }] });

    const page = await createMetaRepository(client).getHistory({ userId: ids.user, limit: 2 });

    expect(page.pageSummary).toEqual({ totalWagered: '0.30', totalPayout: '0.30', netAmount: '0.00', wins: 1, total: 2 });
    expect(page.items[0]).toMatchObject({ wagerAmount: '0.10', verification: { status: 'unavailable' } });
    expect(client.queries.get('game_results')![0].calls).toContainEqual([
      'select', 'id,game,wager_amount,payout_amount,net_amount,multiplier,outcome,server_seed_hash,settled_at',
    ]);
  ...
  it('returns a truthful empty history page', async () => {
    ...
    await expect(createMetaRepository(client).getHistory({ userId: ids.user, limit: 25 })).resolves.toEqual({
      items: [],
      pageSummary: { totalWagered: '0.00', totalPayout: '0.00', netAmount: '0.00', wins: 0, total: 0 },
      nextCursor: null,
      hasMore: false,
    });
  });
  ...
  it('fails closed for database errors and malformed decimal rows', async () => {
    const dbError = new ClientStub({ game_results: [{ data: null, error: { message: 'secret detail' } }] });
    const malformed = new ClientStub({ game_results: [{ data: [historyRow({ payout_amount: 'NaN' })], error: null }] });
    await expect(createMetaRepository(dbError).getHistory({ userId: ids.user, limit: 25 })).rejects.toMatchObject({ code: 'META_DB_ERROR' });
    await expect(createMetaRepository(malformed).getHistory({ userId: ids.user, limit: 25 })).rejects.toBeInstanceOf(MetaRepositoryError);
  });

  it('maps only the public allowlist returned by the opt-in leaderboard RPC', async () => {
    const client = new ClientStub({}, { meta_leaderboard_page: [{ data: [{
      rank: '1', public_name: 'Public', avatar_url: null, metric_value: '90071992547409.91',
      wagered_amount: '90071992547409.91', biggest_win_amount: '10.00', highest_multiplier: '2.0000',
      player_key: 'a'.repeat(64), user_id: ids.user, email: 'private@example.com', balance: '999.00',
    }], error: null }] });

    const page = await createMetaRepository(client, () => new Date('2026-08-06T12:00:00.000Z'))
      .getLeaderboard({ period: 'all-time', metric: 'wagered', limit: 25 });
    const serialized = JSON.stringify(page);

    expect(page.entries).toEqual([{
      rank: 1, publicName: 'Public', avatarUrl: null, metricValue: '90071992547409.91',
      wageredAmount: '90071992547409.91', biggestWinAmount: '10.00', highestMultiplier: '2.0000',
    }]);
    expect(serialized).not.toMatch(/user_id|email|balance/i);
    expect(client.queries.size).toBe(0);
    expect(client.rpcCalls[0]).toMatchObject({ name: 'meta_leaderboard_page', params: { p_limit: 26 } });
  });

  it('uses an explicit bounded admin-user allowlist', async () => {
    const user = (id: string, updatedAt: string): Row => ({
      id, username: 'Player', email: 'player@example.com', balance: '125.50', xp: '100', level: 2,
      rank: 'Bronze', status: 'active', created_at: '2026-08-01T12:00:00.000Z', updated_at: updatedAt,
      password_hash: 'excluded',
    });
    const client = new ClientStub({ users: [{ data: [
      user(ids.user, '2026-08-06T12:00:00.000Z'), user('user_second', '2026-08-05T12:00:00.000Z'),
    ], error: null }] });
    ...
    expect(client.queries.get('users')![0].calls).toContainEqual([
      'select', 'id,username,email,balance,xp,level,rank,status,created_at,updated_at',
    ]);

...
export default function AdminEvalsClient() {
  ...
  const tokenChartData = metrics
    ? [
        { name: 'Input', value: metrics.tokens.input - (metrics.tokens.cachedInput || 0), fill: '#ffd700' },
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: 1200, margin: '0 auto', fontFamily: 'inherit' }}>
      ...
      >
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
            <span style={{ fontSize: '0.75rem', fontWeight: 700, color: 'rgba(255,255,255,0.5)', textTransform: 'uppercase' }}>
              Anfragen & Stabilität
            </span>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginTop: '0.5rem', fontSize: '0.75rem' }}>
            <span style={{ color: '#10b981', fontWeight: 700 }}>
              {metrics?.successRate !== null && metrics?.successRate !== undefined ? `${metrics.successRate}% Erfolg` : '100% Erfolg'}
            </span>
      ...
      >
        ...
        >
          <h3 style={{ fontSize: '0.9rem', fontWeight: 700, color: '#fff', margin: '0 0 1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <Cpu size={16} color="#ffd700" />
            Token-Aufschlüsselung ({selectedWindow === 'last24h' ? '24h' : '7d'})
          </h3>
          <div style={{ width: '100%', height: 220 }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={tokenChartData} layout="vertical" margin={{ top: 10, right: 30, left: 20, bottom: 5 }}>
                <XAxis type="number" stroke="rgba(255,255,255,0.3)" fontSize={11} />
                <YAxis dataKey="name" type="category" stroke="rgba(255,255,255,0.6)" fontSize={11} />
                <Tooltip
                  contentStyle={{ background: '#111', border: '1px solid rgba(212,175,55,0.3)', borderRadius: 8, fontSize: '0.8rem' }}
                />
            ...
            {outcomeChartData.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  <Tooltip
                    contentStyle={{ background: '#111', border: '1px solid rgba(255,255,255,0.15)', borderRadius: 8, fontSize: '0.8rem' }}
                  />
                </PieChart>
              </ResponsiveContainer>
            ) : (
      ...
      >
        ...
        >
          <h2 style={{ fontSize: '0.95rem', fontWeight: 700, color: '#fff', margin: 0, display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              ...
              {!feedback || feedback.recentFeedback.length === 0 ? (
                ...
                feedback.recentFeedback.map((item) => (
                  <tr key={item.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                    <td style={{ padding: '0.8rem 1rem', verticalAlign: 'middle' }}>
                      <span
                        style={{
                          ...
                          fontSize: '0.75rem',
                          fontWeight: 700,
                          background: item.rating === 1 ? 'rgba(16,185,129,0.15)' : 'rgba(239,68,68,0.15)',
                    ...
                    <td style={{ padding: '0.8rem 1rem', color: '#ffd700', fontSize: '0.75rem', fontFamily: 'monospace' }}>
                      {item.category || 'Allgemein'}
                    </td>
                    <td style={{ padding: '0.8rem 1rem', color: 'rgba(255,255,255,0.8)', fontSize: '0.75rem' }}>
                      {item.comment || '—'}
                    </td>
                  </tr>
                ))

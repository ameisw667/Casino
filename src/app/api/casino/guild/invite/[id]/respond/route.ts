...
import { z } from 'zod';
import { respondToInvite, respondInviteInputSchema, GuildServiceError } from '@/lib/casino/guild-service';
...
export async function POST(
  ...
  try {
    ...
    const rate = await enforceRateLimit(getClientIdentifier(request, userId), 'guild-invite-respond', 10, 60);
    if (!rate.success) {
    ...
    const parsed = respondInviteInputSchema.safeParse(body);
    if (!parsed.success) {
      return NextResponse.json(
        { error: parsed.error.issues[0]?.message || 'Invalid response action' },
        { status: 400, headers: { ...PRIVATE_HEADERS, ...rateLimitHeaders(rate) } },
      );
    }

    const result = await respondToInvite(userId, id, parsed.data.action);
    ...
    CasinoLogger.error('API/GuildInviteRespond', 'Failed to respond to invite', error);
    return NextResponse.json({ error: 'Invite response unavailable' }, { status: 503, headers: PRIVATE_HEADERS });
  }
}

...
export async function GET(request: Request) {
  try {
    ...
    const { data: users, error: usersError } = await admin.from('users').select('id, balance, xp');
    ...
    const hourlyBins: Record<number, { wagered: number; payout: number }> = {};
    for (let i = 0; i < 24; i++) {
      hourlyBins[i] = { wagered: 0, payout: 0 };
    }
    ...
    if (!txsError && txs) {
      for (const t of txs) {
        ...
        if (t.type === 'bet' || t.type === 'bet_settled' || Number(t.amount) < 0) {
          totalWagered += amt;
          hourlyBins[hour].wagered += amt;
        } else if (t.type === 'win' || t.type === 'payout' || Number(t.amount) > 0) {
          totalPayout += amt;
          hourlyBins[hour].payout += amt;
        }
      }
    }
    ...
    // Build 24h chart bins from actual transaction timestamps
    const wagerData = Array.from({ length: 24 }, (_, i) => ({
      hour: `${i.toString().padStart(2, '0')}:00`,
      wagered: Math.round(hourlyBins[i].wagered),
      profit: Math.round(hourlyBins[i].wagered - hourlyBins[i].payout),
    }));

    const stats = [
      {
        label: 'Total Wagered',
        value: `$${totalWagered.toLocaleString('en-US')}`,
        change: '+100%',
        icon: 'Wallet',
        color: '#D4AF37',
      },

'use client';

import React from 'react';
import { motion } from 'framer-motion';
import type { Card } from './PlayingCard';
import { VintageCardBack } from './VintageCardBack';

interface BlackjackCard3DProps {
  card: Card | null;
  faceDown?: boolean;
  isPeeking?: boolean;
  size?: 'sm' | 'md' | 'lg';
  variant?: 'burgundy' | 'navy' | 'obsidian';
}

const SUIT_SYMBOLS: Record<string, string> = {
  spades: '♠',
  clubs: '♣',
  hearts: '♥',
  diamonds: '♦',
};

export function BlackjackCard3D({
  card,
  faceDown = false,
  isPeeking = false,
  size = 'md',
  variant = 'burgundy',
}: BlackjackCard3DProps) {
  const isRed = card?.suit === 'hearts' || card?.suit === 'diamonds';
  const suitIcon = card ? SUIT_SYMBOLS[card.suit] : '♠';

  const width = size === 'sm' ? 68 : size === 'lg' ? 104 : 88;
  const height = size === 'sm' ? 96 : size === 'lg' ? 148 : 124;

  return (
    <div
      style={{
        width: `${width}px`,
        height: `${height}px`,
        perspective: '1200px',
        userSelect: 'none',
        position: 'relative',
      }}
    >
      <motion.div
        animate={{
          rotateY: isPeeking ? 168 : faceDown ? 180 : 0,
          rotateX: isPeeking ? 16 : 0,
          rotateZ: isPeeking ? 2.5 : 0,
          y: isPeeking ? -10 : 0,
          x: isPeeking ? -4 : 0,
          scale: isPeeking ? 1.04 : 1,
        }}
        transition={{
          duration: isPeeking ? 0.28 : 0.36,
          ease: [0.16, 1, 0.3, 1],
        }}
        style={{
          width: '100%',
          height: '100%',
          position: 'relative',
          transformStyle: 'preserve-3d',
          borderRadius: '12px',
          boxShadow: isPeeking
            ? '0 24px 48px rgba(0, 0, 0, 0.95), 0 0 24px rgba(212, 175, 55, 0.35)'
            : '0 18px 36px -4px rgba(0, 0, 0, 0.85), 0 8px 16px rgba(0, 0, 0, 0.6), 0 0 1px rgba(255, 255, 255, 0.25)',
        }}
      >
        {/* ======================================================== */}
        {/* FRONT FACE: 1:1 identisch mit der beliebten klassischen Karte */}
        {/* ======================================================== */}
        <div
          style={{
            position: 'absolute',
            inset: 0,
            backfaceVisibility: 'hidden',
            borderRadius: '12px',
            background: 'linear-gradient(145deg, #FFFFFF 0%, #F8F8FA 60%, #EEEEF2 100%)',
            border: '1.5px solid rgba(212, 175, 55, 0.55)',
            boxShadow: 'inset 0 1px 3px rgba(255,255,255,1), inset 0 -1px 2px rgba(0,0,0,0.12)',
            padding: '8px',
            display: 'flex',
            flexDirection: 'column',
            justifyContent: 'space-between',
            color: isRed ? '#DC2626' : '#0F172A',
            overflow: 'hidden',
          }}
        >
          {/* Top-Left Corner */}
          <div
            style={{
              display: 'flex',
              flexDirection: 'column',
              alignItems: 'center',
              width: 'fit-content',
            }}
          >
            <span
              style={{
                fontSize: size === 'sm' ? '0.85rem' : '1.1rem',
                fontWeight: 900,
                lineHeight: 1,
              }}
            >
              {card?.value}
            </span>
            <span
              style={{
                fontSize: size === 'sm' ? '0.75rem' : '0.95rem',
                lineHeight: 1,
                marginTop: '1px',
              }}
            >
              {suitIcon}
            </span>
          </div>

          {/* Center Graphic with Drop Shadow & Court Labels */}
          <div
            style={{
              position: 'absolute',
              top: '50%',
              left: '50%',
              transform: 'translate(-50%, -50%)',
              display: 'flex',
              flexDirection: 'column',
              alignItems: 'center',
              justifyContent: 'center',
            }}
          >
            <span
              style={{
                fontSize: size === 'sm' ? '1.8rem' : size === 'lg' ? '2.8rem' : '2.4rem',
                lineHeight: 1,
                filter: isRed
                  ? 'drop-shadow(0 2px 4px rgba(220, 38, 38, 0.25))'
                  : 'drop-shadow(0 2px 4px rgba(15, 23, 42, 0.25))',
              }}
            >
              {suitIcon}
            </span>
            {card?.value && ['J', 'Q', 'K', 'A'].includes(card.value) && (
              <span
                style={{
                  fontSize: '0.62rem',
                  fontWeight: 900,
                  letterSpacing: '1px',
                  color: isRed ? '#991B1B' : '#334155',
                  marginTop: '2px',
                  fontFamily: 'monospace',
                }}
              >
                {card.value === 'A'
                  ? 'ACE'
                  : card.value === 'K'
                    ? 'KING'
                    : card.value === 'Q'
                      ? 'QUEEN'
                      : 'JACK'}
              </span>
            )}
          </div>

          {/* Bottom-Right Corner (Inverted) */}
          <div
            style={{
              display: 'flex',
              flexDirection: 'column',
              alignItems: 'center',
              width: 'fit-content',
              alignSelf: 'flex-end',
              transform: 'rotate(180deg)',
            }}
          >
            <span
              style={{
                fontSize: size === 'sm' ? '0.85rem' : '1.1rem',
                fontWeight: 900,
                lineHeight: 1,
              }}
            >
              {card?.value}
            </span>
            <span
              style={{
                fontSize: size === 'sm' ? '0.75rem' : '0.95rem',
                lineHeight: 1,
                marginTop: '1px',
              }}
            >
              {suitIcon}
            </span>
          </div>
        </div>

        {/* ======================================================== */}
        {/* BACK FACE: Nostalgischer Vintage Heritage Rider Back       */}
        {/* ======================================================== */}
        <div
          style={{
            position: 'absolute',
            inset: 0,
            backfaceVisibility: 'hidden',
            transform: 'rotateY(180deg)',
            borderRadius: '12px',
            overflow: 'hidden',
          }}
        >
          <VintageCardBack size={size} variant={variant} />
        </div>
      </motion.div>
    </div>
  );
}

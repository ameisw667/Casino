...
export type RouletteStrategyPreset =
  'VOISINS' | 'TIERS' | 'ORPHELINS' | 'RED_BLACK_HEDGE' | 'ZERO_HEDGE';

interface RouletteStrategyBarProps {
  showRacetrack: boolean;
  onToggleRacetrack: () => void;
  onApplyPreset: (preset: RouletteStrategyPreset) => void;
  lastWinAmount: number | null;
  maxPotentialWin: number;
}

const STRATEGY_PRESETS: { label: string; key: RouletteStrategyPreset }[] = [
  { label: 'ROT DECKUNG', key: 'RED_BLACK_HEDGE' },
  { label: 'VOISINS DU ZÉRO', key: 'VOISINS' },
  { label: 'TIERS DU CYLINDRE', key: 'TIERS' },
  { label: 'ORPHELINS', key: 'ORPHELINS' },
  { label: 'ZERO HEDGE', key: 'ZERO_HEDGE' },
];
...
export function RouletteStrategyBar({
  showRacetrack,
  onToggleRacetrack,
  onApplyPreset,
  lastWinAmount,
  maxPotentialWin,
}: RouletteStrategyBarProps) {
  return (
    <div
      style={{
        display: 'flex',
    ...
    >
      ...
      {/* Mitte: Zentrierte, einzeilige Strategy Pills */}
      <div
        style={{
          display: 'flex',
          alignItems: 'center',
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
        ...
        >
          ...
          >
            POTENZIAL: +${lastWinAmount ?? (maxPotentialWin > 0 ? maxPotentialWin : 0)}
          </span>
        </div>
      </div>
    </div>

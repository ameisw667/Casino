...
export const RouletteBoard: React.FC<RouletteBoardProps> = ({
  ...
  const renderNumberTile = (num: number) => {
    ...
    return (
      <motion.div
        ...
        whileTap={{ scale: 0.95 }}
        className={`relative flex h-14 cursor-pointer items-center justify-center border border-white/5 text-lg font-black transition-all duration-300 select-none md:h-16 ${isWinner ? 'shadow-glow-white z-20 scale-105 ring-4 ring-white' : ''} ${highlighted ? 'z-10' : ''}`}
        style={{
          background:
            data.c === 'RED'
  ...
  const renderSideBet = (label: string, type: BetType, className: string = '') => {
    ...
    return (
      <motion.div
        ...
        className={`glass-subtle relative flex cursor-pointer items-center justify-center border border-white/5 text-[10px] font-black tracking-widest uppercase transition-all hover:z-10 md:text-xs ${className} ${active ? 'ring-2 ring-white/20 brightness-150' : ''}`}
        style={{
  ...
  return (
    <div className="no-scrollbar flex w-full flex-col gap-8 overflow-x-auto pb-4">
      {/* Racetrack Visual (Simplified) */}
      <div className="flex justify-center gap-1 opacity-60 transition-opacity hover:opacity-100">
        <div className="flex h-12 items-center gap-1 overflow-hidden rounded-full border border-white/10 bg-black/40 px-6">
          {WHEEL_ORDER.map((num, i) => (
            <div
              key={i}
              className={`h-full w-4 transition-all duration-300 ${isHighlighted(num.n) ? 'scale-y-110 brightness-150' : ''}`}
              style={{
                background:
                  num.c === 'RED'
                    ? 'hsl(var(--error))'
      ...
      <div className="grid min-w-[850px] grid-cols-[70px_1fr_90px] gap-1 rounded-2xl border border-white/5 bg-black/40 p-1 backdrop-blur-xl">
        ...
        >
          ...
          {currentBets.find((b) => b.type.type === 'STRAIGHT' && b.type.value === 0) && (
            <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
              <Chip
                amount={
                  currentBets.find((b) => b.type.type === 'STRAIGHT' && b.type.value === 0)!.amount
                }
                size={36}
                stacked
                vipLevel={vipLevel}
        ...
        {/* Dozens */}
        <div className="col-start-2 mt-1 grid grid-cols-3 gap-1">
        ...
        {/* Outside Bets */}
        <div className="col-start-2 mt-1 grid grid-cols-6 gap-1">
          ...
          {renderSideBet(
            ...
            'h-12 md:h-14 bg-[hsl(var(--error))] text-white border-none',
          )}
          {renderSideBet(
            'BLACK',
            { type: 'COLOR', value: 'BLACK' },
            'h-12 md:h-14 bg-black text-white border-none',
          )}

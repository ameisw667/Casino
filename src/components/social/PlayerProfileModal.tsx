...
export function PlayerProfileModal({ user, isOpen, onClose }: PlayerProfileModalProps) {
  ...
  const [randomData, setRandomData] = React.useState({ level: 5, xp: 5000, totalWagered: 2500, biggestWin: 100 });
        ...
        <>
          ...
          <motion.div 
            ...
            style={{
              ...
              width: '90%', maxWidth: '480px', background: '#1a2c38',
              borderRadius: '24px', border: '1px solid rgba(255,255,255,0.05)',
              zIndex: 2001, overflow: 'hidden', boxShadow: '0 20px 50px rgba(0,0,0,0.5)'
            }}
          >
            {/* Header / Banner */}
            <div style={{ height: '100px', background: `linear-gradient(45deg, ${tier.color}33, transparent)`, position: 'relative' }}>
              <button 
                onClick={onClose}
                style={{ position: 'absolute', right: '16px', top: '16px', background: 'rgba(0,0,0,0.3)', border: 'none', borderRadius: '50%', padding: '8px', color: '#fff', cursor: 'pointer' }}
              >
                <X size={20} />
              </button>
            </div>
            ...
            <div style={{ padding: '0 24px 24px', marginTop: '-40px' }}>
              <div style={{ display: 'flex', alignItems: 'flex-end', gap: '16px', marginBottom: '24px' }}>
                <div style={{ 
                  width: '80px', height: '80px', borderRadius: '20px', background: '#243b4a',
                  border: `4px solid #1a2c38`, display: 'flex', alignItems: 'center', justifyContent: 'center',
                  fontSize: '2rem', boxShadow: '0 10px 20px rgba(0,0,0,0.2)'
                }}>
                  {user[0].toUpperCase()}
                </div>
                <div style={{ paddingBottom: '8px' }}>
                  <h2 style={{ fontSize: '1.5rem', fontWeight: 950, marginBottom: '4px' }}>{user}</h2>
                  <div style={{ display: 'flex', gap: '8px' }}>
                    ...
                    <span style={{ fontSize: '0.75rem', color: '#b1bad3', display: 'flex', alignItems: 'center', gap: '4px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '24px' }}>
                ...
                <div className="glass-card" style={{ padding: '16px', borderRadius: '16px', background: 'rgba(255,255,255,0.02)' }}>
                  <div style={{ color: '#b1bad3', fontSize: '0.7rem', fontWeight: 800, marginBottom: '4px', textTransform: 'uppercase' }}>Best Win</div>
              ...
              <div style={{ marginBottom: '24px' }}>
                <h3 style={{ fontSize: '0.85rem', fontWeight: 900, color: '#b1bad3', marginBottom: '12px', textTransform: 'uppercase' }}>Recent Achievements</h3>
                <div style={{ display: 'flex', gap: '8px' }}>
                  {['🎯', '🐋', '🔥'].map((icon, i) => (
                    <div key={i} style={{ 
              ...
              <button 
                ...
                style={{ width: '100%', padding: '12px', borderRadius: '12px', fontWeight: 800, gap: '8px' }}
                onClick={() => {}}
              >
                Tip Player
              </button>

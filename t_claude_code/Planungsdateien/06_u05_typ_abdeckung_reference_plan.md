...
> **Status:** Execution-Ready · **Stand:** 2026-09-14 · **Owner:** LLM (Jan nur bei Memory-Schreib-Gate) · **Scope:** Vorbereitung der 4 verifizierten reference-Memory-Einträge (Fertigentwürfe + Secrets-Ausschluss-Checkliste) als Freigabegrundlage; keine Memory-Änderung ohne Gate G1.
...
| ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------- | ---------- | ---------------------------- | ------------------------------------------------------------------------------------- |
| L0     | Baseline verifizieren: Memory-Ordner scannen (`metadata.type` je Datei), genau 1× reference bestätigen; alle 4 Kandidaten-Belegstellen im Repo (K1–K4, §2a) als vorhanden bestätigen                                                                                                                  | read-only: `C:\Users\hambu\.claude\projects\V--VibeCoding-Casino\memory\*`, `CLAUDE.md`, `docs/archive/01_github.md`, `docs/archive/01_SentryCLI_SentryMCP.md`, `docs/auth/12_middleware_proxy_csp.md` | Sequenziell                 | 🔴 Geplant | LLM                          | Report: Typ-Verteilung + 4 Belegstellen (Datei:Zeile) ohne Abweichung                 |
| L1     | Fertigentwürfe finalisieren (§2a ist bereits ausformuliert, nur Anpassungen nach L0-Befund); Secrets-Ausschluss-Checkliste §2b über jeden Entwurf laufen lassen (4× PASS)                                                                                                                             | diese Plan-Datei (`06_u05_...plan.md`)                                                                                                                                                                 | Sequenziell                 | 🔴 Geplant | LLM                          | 4 vollständige Entwürfe (Frontmatter + Body) je ohne „TODO" und je 1 Checklisten-PASS |
| L2     | **Gate G1 (Jan):** 4 Entwürfe vorlegen, Freigabe einholen; bei Ja: 4 reference-Einträge in den Memory-Ordner schreiben (`originSessionId`/`modified` wie beim Vorbild-Eintrag ergänzen) + je 1 Indexzeile in `MEMORY.md`; bei Nein: Entwürfe unverändert als Entscheidungsgrundlage archiviert lassen | Memory-Ordner + `MEMORY.md`                                                                                                                                                                            | Sequenziell (nur nach Gate) | 🔴 Geplant | Jan (Gate) + LLM (Schreiben) | `metadata.type: reference` in 4 neuen Dateien; 4 Indexzeilen vorhanden                |
| L3     | Niveau-Rückschreibung: `01_6_05` (Subsub 1/3) + Parent [`../01_6_memory_files.md`](../01_6_memory_files.md) Position 5 neu bewerten; Plan-Status → `Executed (archiviert)` **in-place** (kein Worldmap-Eintrag — Abschnitt „Aktive Pläne" existiert nicht, verifiziert 2026-09-17)                    | `../01_6_05_*.md`, `../01_6_memory_files.md`                                                                                                                                                           | Sequenziell                 | 🔴 Geplant | LLM                          | Schnitt-Update dokumentiert + Statuszeile in dieser Datei                             |
...
- Keine Änderung am Memory-Schema, an `metadata`-Feldkonventionen oder an der Vorbild-Datei `openai_image_video_pricing_reference.md`.

...
> **Status:** Executed (2026-09-14; Analyse-Teil vollumfänglich; **Option-Wahl am 2026-09-16 abgeschlossen — Gate 2 = X3**, Jan hat die Wahl delegiert) · **Stand:** 2026-09-16 · **Owner:** LLM (Jan nur bei Gate) · **Scope:** Analyse und Refactor-Vorbereitung der 2 Hotspots (Crash-Loop-Duplikation, >800-Zeilen-Kandidaten); keine Implementation von Game-Logik-Refactors ohne separate Option-Gate-Entscheidung, keine Mess-Infrastruktur (Plan 09).
...
- **Messbasis (nachgemessen 2026-09-16):** 1.044 TS/TSX-Dateien unter `src/`; 3 > 800: `src/types/database.types.ts` (2.054, generiert — bewusst NICHT refactorbar), `src/components/casino/games/crash/useCrashGameLoop.ts` (982), `src/lib/casino/wallet.ts` (880, Money-Pfad). Der frühere vierte Ausreißer `src/store/__tests__/useCasinoStore.test.ts` (1.254, Zahlen der Erstrunde 2026-09-14) wurde durch **03a-R09 am 2026-09-14 aufgeteilt** (5 Module + 2 Helper, größtes Modul 394 Z.) und ist damit kein Hotspot mehr.
- **Duplikations-Kern (Bottleneck #3):** `crash/useCrashGameLoop.ts` (982) vs. `crash-multiplayer/useCrashMultiplayerGameLoop.ts` (782) — jede Crash-Task-Änderung zwingt LLM-Reads beider Dateien (≈ 1.764 Zeilen).
...
`Geplant` → Jan-Freigabe (Gate: L1-Option-Wahl — **am 2026-09-16 abgeschlossen: X3, 4,20**) → `Execution-Ready` → `In Execution` → nach L3 `Executed (archiviert)`; die Umsetzung der gewählten Option läuft in einem separaten Implementation-Plan.

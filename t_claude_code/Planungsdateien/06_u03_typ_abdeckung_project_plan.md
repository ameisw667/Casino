...
> **Status:** Execution-Ready · **Stand:** 2026-09-14 · **Owner:** LLM (Jan nur bei Memory-Schreib-Gate) · **Scope:** project-Typ im Memory-Ordner schließen (3 neue project-Memories + 1 Nachtrag am Altbestand + Trigger-Cross-Verweis), ohne feedback/reference-Einträge anzufassen.
...
| :----: | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :--------------------------------------------------------------------------------------------- | :-------------------: | :--------: | :-------------------------------: | :--------------------------------------------------------------------------------------------------------------------- |
| **L0** | **3 project-Memory-Kandidaten anlegen** (fertige Entwürfe aus §2a unverändert als Dateien schreiben, inkl. Index-Zeile je Eintrag in `MEMORY.md`)                                                                                                  | `C:\Users\hambu\.claude\projects\V--VibeCoding-Casino\memory\` + `MEMORY.md`                   |      Sequenziell      | 🔴 Geplant | LLM (**Gate G1** vor jedem Write) | 3 neue Dateien + 3 Index-Zeilen; Frontmatter je Datei vollständig (`name`/`description`/`metadata.type: project`)      |
| **L1** | **Nachtrag Altbestand** `vip-rank-supabase-outsourcing.md` (Ergänzungszeilen-Entwürfe aus §2b, Update-Layer-Stil)                                                                                                                                  | `C:\Users\hambu\.claude\projects\V--VibeCoding-Casino\memory\vip-rank-supabase-outsourcing.md` | Sequenziell (nach L0) | 🔴 Geplant |         LLM (**Gate G1**)         | Update-Absatz + Aktuelle-Einordnung ergänzt; Ursprungsregel unverändert bewahrt (Muster `no-visual-check-frontend.md`) |
| **L2** | **Trigger-Regel ablegen** (§2c-Inhalt) — primärer Ablageort ist [`06_u07_pflegekadenz_plan.md`](06_u07_pflegekadenz_plan.md) §2a/§2b (**existiert**, am 2026-09-17 verifiziert); hier bleibt nur eine 1-Zeilen-Verweisung, keine Doppelverankerung | `t_claude_code/Planungsdateien/`                                                               |      Sequenziell      | 🔴 Geplant |                LLM                | Regeltext existiert an genau **einem** Ort (06_u07 §2a); 06_u03 §2c = Verweiszeile; beide Gegenverweise klicken        |
| **L3** | **Abschlussprüfung & Niveau-Rückschreibung** in [`../01_6_03_typ_abdeckung_project.md`](../01_6_03_typ_abdeckung_project.md)                                                                                                                       | `t_claude_code/01_6_03_typ_abdeckung_project.md`                                               |      Sequenziell      | 🔴 Geplant |                LLM                | Niveau-Rückschreibung §5 ausgefüllt; Deckung neu berechnet (1 → 4 project-Einträge)                                    |
...
1-Absatz-Regel (Ablage: primär Plan [`06_u07_pflegekadenz_plan.md`](06_u07_pflegekadenz_plan.md); dieser Plan hier verweist nur darauf — **keine Doppelverankerung**; solange 06_u07 noch nicht angelegt ist, bleibt der Text ausschließlich hier stehen und wird bei Anlage von 06_u07 dorthin transferiert):

> Ein project-Memory-Kandidat entsteht genau dann, wenn eine Session mit **einem** der drei Muster endet: (1) eine lernreiche Commit-Welle (≥ 5 thematisch zusammenhängende Commits mit Architektur-/Infrastruktur-Ergebnis — Beispiel: DB-Pooling-Runde 2026-09-13, K0–K8-Welle 2026-09-14), (2) eine abgeschlossene Options-Gate-Entscheidung nach `xx_sop/01_workflow_jan_option_gate.md`, deren gewählte Option künftige Sessions bindet, oder (3) eine Security-/DB-Härtungsrunde (z. B. Chaos-Hardening, Migrations-Rolle). In allen drei Fällen formuliert das LLM nach Abschluss **einen** Entwurf (Fakt / Warum / How to apply) und legt ihn Jan als Schreibvorschlag vor — **nie Auto-Write** (globale Regel „Write to memory **only when I explicitly ask**"). Jan-Freigabe = Gate G1; erst danach Datei anlegen und Index-Zeile ergänzen.

Diese Regel adressiert Subsub 4 (Kandidaten-Identifikation, kein Trigger-Mechanismus) der Bewertungs-Basis; die Kadenz-/Frequenzseite gehört zu 06_u07 (Cross-Verweis statt Duplizierung — 01_6_07 Subsub 3 behandelt denselben Trigger-Aspekt auf Verhaltensebene).
...
- **Jan-Gates:** **Gate G1** — Freigabe für L0 (3 Writes + 3 Index-Zeilen) und L1 (1 Nachtrag). Am 2026-09-17 als **ein Sammel-Batch** mit den übrigen Typ-Abdeckungs-Plänen zusammengelegt (Go noch offen). Ohne Go wird nichts geschrieben; das Ausführungs-LLM liefert die Entwürfe (§2) und hält an.

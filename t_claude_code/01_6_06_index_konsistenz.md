...
- **Zeile 5** (image-generation) ↔ „…gpt-image-2 used for all OpenAI image generation…, never … gpt-image-1" — übereinstimmend; Index lässt die Body-Ausnahme („gpt-image-1-mini … ask Jan first", `image_generation_model_preference.md:15`) weg — vereinfachend, aber nicht falsch.

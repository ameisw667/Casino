...
> **Status:** Executed (2026-09-08, L1–L4; Endabnahme via Ergebnis-Links) · **Stand:** 2026-09-08 · **Owner:** LLM (Jan nur bei Gate) · **Scope:** Ausschließlich `Sparkles` (35 Render-Stellen, §18.3 Zeile 1). Keine anderen 18.3-Icons, keine Wallet-/Settlement-Logik.
...
| L1     | AI/Guide-Integration (6 Stellen)                  | `GuideTriggerButton.tsx`, `DiceCenterStage.tsx`, `CrashStage.tsx`, `CrashTutorial.tsx`, `SlotsCenterStage.tsx`, `BlackjackLeftSidebar.tsx`                      | 🟢 Erledigt (2026-09-08) |      LLM      | Aktive Stellen migriert: GuideTriggerButton:59, CrashTutorial:52, SlotsCenterStage:351, BlackjackLeftSidebar:108 + Extras (GameCoPilotHud ×3, GuideMessageList:329, GuideImagePreview:72, TutorialModal:46, GuideSidebar-Fallback → `BookOpen`, SL-Tag → `Cherry`). **Scope-Korrektur:** DiceCenterStage:223 + CrashStage:133 hatten kein aktives `Sparkles` mehr (früher migriert, Plan-Listen-Drift) |                                                                                                                                                                                                          |
| L2     | Bonus/Promo-Integration (3 Stellen)               | `JackpotPulseCard.tsx`, `InteractiveArcadeGrid.tsx`, `VipPersonalRecords.tsx`, `LiveHighrollerTickerBar.tsx` (Jackpot-Zeile)                                    | 🟢 Erledigt (2026-09-08) |      LLM      | Alle 4 migriert + Extra `HighrollerWinDetailModal.tsx:50` (case 'jackpot' → Bonus-Asset). **Neubefund:** `InteractiveArcadeGrid` wird nirgends gerendert (nur Typ-Exports via `BentoArcadeCells`) — Migration trotzdem vollzogen |                                                                                                                                                                                                      |
| L3     | Reassignment Rest-Stellen (nicht AI, nicht Bonus) | `HistoryTableStream.tsx:69`, `BetReceiptModal.tsx:386`                                                                                                          | 🟢 Erledigt (2026-09-08) |      LLM      | Slots-History → `Cherry`, Prüfcode → `Copy`; Rest-Grep: 0 Verwendungen außerhalb `/testing` (Sandbox) + Admin (planmäßig out of Scope); `GameShowcaseCard:273` (LIVE STATUS, weder AI noch Bonus) → `Activity` |                                                                                                                                                                                                                           |
| L4     | Verifikation & Doku-Update                        | `T_FRONTEND/ALLE_ICONS_BUTTONS_ANALYSE.md`, `public/images/CHANGELOG.md`                                                                                          | 🟢 Erledigt (2026-09-08) |      LLM      | typecheck 0 Fehler, lint 0 Errors, §18.3-Zeile aktualisiert; `npm test`/`npm run build` im Gesamt-DoD der Runde (alle Pläne teilen denselben Baum) |                                                                                                                                                                                                                                   |

---

## 2 — Kontext-Koffer

### 2.1 Betroffene Stellen (aus §18.3-Inventur, `T_FRONTEND/ALLE_ICONS_BUTTONS_ANALYSE.md`)
...
- **Schritte:** 1. `npm run typecheck`, `npm run lint`, `npm test`, `npm run build`. 2. `git diff` auf unbeabsichtigte Dateien prüfen. 3. `T_FRONTEND/ALLE_ICONS_BUTTONS_ANALYSE.md` §18.3-Zeile `Sparkles` von „Ja"-Referenz auf „umgesetzt, siehe CHANGELOG" ergänzen (Format wie bestehende Zeilen).
...
> **Status:** 🟡 Vorschlag — wartet auf Jans Freigabe zur Regenerierung. Keine API-Kosten ausgelöst, L0-Status in §1 bleibt bewusst unverändert auf „Erledigt" (bezieht sich auf die tatsächlich generierten Dateien), bis eine Regenerierung stattfindet.
> **Anlass:** Sichtprüfung der L0-Ergebnisse gegen die reale Ziel-Einsatzgröße (13–16px Guide-Button bzw. 11–13px Bonus-Kontexte, nicht die 1024px-Vorschau) ergab Lesbarkeits-/Eindeutigkeitsprobleme.

| Asset | Befund | Alt-Prompt (aktuell generiert) | Neu-Prompt (Vorschlag) |
| :--- | :--- | :--- | :--- |
| `icon-ai-guide-quantum-gold` | Liest sich als generischer Diamant-Kompass/Schneekristall, kaum von `star-rating` unterscheidbar; 4 Hauptstrahlen laufen bis an den Bildrand (Clipping-Risiko beim Verkleinern). | „a single stylized compass rose fused with a faceted diamond core at its center, four sharp radiant light beams extending outward from the diamond, symbolizing an intelligent guiding assistant, precise geometric facets" | „a single faceted crystal orb with a luminous glowing iris-lens embedded at its exact center, four short bold diamond-tipped compass spikes fused flush onto the orb's equator forming one unified compact silhouette, no thin rays or lines extending beyond the object's outer edge, symbolizing an intelligent AI guide core, centered, full view without clipping, simplified bold facets for small-scale clarity" |
| `icon-promo-bonus-quantum-gold` | Ergebnis liest sich als gedeckelter Krug/Gefäß statt Bonus-Assoziation; über ein Dutzend Einzelmünzen verstößt gegen die Icon-Kategorie-Ausschlussregel „no multiple objects" (`src/lib/design-assets/style-preset.ts:24`) und verschwimmt bei 11–13px. | „a bursting cluster of stacked gold coins with radiant spark bursts exploding outward from the center, symbolizing a jackpot bonus reward, dynamic upward motion" | „a single open treasure chest overflowing with three large stacked gold coin piles, one radiant oversized coin tilted forward at the front bearing a bold dollar-sign embossment, sharp golden spark bursts radiating from the coin piles, symbolizing a jackpot bonus reward, compact silhouette with at most 3-4 large coin shapes, no lid, no small scattered coins, centered, full view without clipping" |

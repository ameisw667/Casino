...
> **Status:** Executed (2026-09-08, L1–L4; Endabnahme via Ergebnis-Links) · **Stand:** 2026-09-08 · **Owner:** LLM (Jan nur bei Gate) · **Scope:** Ausschließlich `Trophy` (§18.3 Zeile 3). Keine anderen 18.3-Icons.
...
| L1     | Win-Integration (3 Stellen)                                          | `ToastContainer.tsx`, `ProgressiveJackpotSection.tsx`, `LiveActivityFeedV2.tsx`                                                                                                                                  | 🟢 Erledigt (2026-09-08) |      LLM      | typecheck 0 Fehler; Toast 20px, Jackpot-Header 14px, Feed 14px/12px 1:1 migriert                                                     |
| L2     | Turnier-Integration (1 Stelle)                                       | `DailyTournamentTeaser.tsx`                                                                                                                                                                                      | 🟢 Erledigt (2026-09-08) |      LLM      | typecheck 0 Fehler; 13px 1:1 (Dead-Render-Pfad, siehe §2.1.1)                                                                       |
| L3     | Record/Rang-Integration (3 Stellen) + Achievements-Header-Entfernung | `VipPersonalRecords.tsx`, `MobileNav.tsx`, `FavoriteGameCard.tsx`, `VaultAchievements.tsx`                                                                                                                       | 🟢 Erledigt (2026-09-08) |      LLM      | Kein `Trophy`-Import mehr in diesen 4 Dateien; MobileNav vorher am 390px-Viewport verifiziert (Crop `18_3_trophy_mobilnav_vorher_check.png`) |
...
- **Dead-Render-Pfade (Migration trotzdem vollzogen, dokumentiert):** `ProgressiveJackpotSection.tsx:117` und `DailyTournamentTeaser.tsx:77` liegen in Default-Export-Komponenten, die nirgends gerendert werden — die Home-Lobby nutzt nur die benannten Exports `RollingJackpotDisplay` (BentoJackpotCells) bzw. `PRIZE_BY_RANK`/`RANK_STYLE` (BentoStripCells). Ebenso `LiveActivityFeed.tsx:221` (nicht-V2-Feed, nirgends importiert) und `GlobalLeaderboard.tsx:43` (nirgends importiert).
- **Neubefunde (migriert):** `BigWinOverlay.tsx:206` (3D-rotierender Trophy, Win-Moment) → `icon-trophy-win` 76/116px inkl. Glow-Filter; `RankBenefitsModal.tsx:81` (Rank-Header) → `icon-trophy-record` 20/24px; `SlotsPaytable.tsx:46` + `app/games/slots/v2/page.tsx:283` (Paytable-Header) → `icon-trophy-win` 16px/18px; `NotificationCenter.tsx:245` (big_win-Notification) → `icon-trophy-win` 18px; `HeroSection.tsx:157` + `HeroSectionV2.tsx:325` („FIRST DEPOSIT BONUS", Dead-Paths, Bonus-Kontext) → `icon-promo-bonus` 14px (konsistent zu Plan 27).
- **Bewusste Ausnahme:** `MainLayout.tsx:309` (Desktop-Sidebar-Nav „Leaderboard") bleibt Lucide-`Trophy` — der Vertragstest `MainSidebar.test.ts:18` spiegelt genau dieses Icon; Reassignment nur in einem eigenen Nav-Icon-Plan.
- **Out of Scope unverändert:** `/v2`-Sandbox (`V2Sidebar`, `V2Header`), `/testing` (`SidebarOptionsClient`, `NeonArcadeDashboardView` — wird nur unter `/testing` gerendert).
...
> **Anlass:** Sichtprüfung gegen die reale Ziel-Einsatzgröße (12–20px, u. a. Mobile-Nav 12px). `trophy-win` und `trophy-tournament` sind nur Feinschliff; `trophy-record` hat ein echtes Unterscheidungsproblem, da alle 3 Assets dieselbe Cup-Außensilhouette teilen und nur ein Interieur-Detail variiert — bei 12–14px praktisch nicht mehr unterscheidbar.

| Asset | Befund | Alt-Prompt (aktuell generiert) | Neu-Prompt (Vorschlag) |
| :--- | :--- | :--- | :--- |
| `icon-trophy-win-quantum-gold` | Bereits sehr sauber — nur Politur gegen feine Basis-Facetten bei 14–20px. | „a single classic winners trophy cup with a rounded bowl and two curved handles, radiant victory glow emanating from the rim, symbolizing an immediate win moment" | „a single classic winners trophy cup with a smooth unadorned rounded bowl and two curved handles, radiant victory glow emanating from the rim, symbolizing an immediate win moment, minimal base detail, thick clean silhouette for small-scale legibility" |
| `icon-trophy-tournament-quantum-gold` | Lorbeerkranz als einziges Unterscheidungsmerkmal zu `trophy-win` — Einzelblätter könnten bei 13px verschwimmen. | „a tall narrow championship cup trophy wrapped with a laurel wreath crown around its neck, standing on a stepped pedestal base, symbolizing tournament competition" | „a tall narrow championship cup trophy wrapped with a bold thick laurel wreath crown around its neck, standing on a stepped pedestal base, symbolizing tournament competition, wreath rendered as one continuous chunky ring shape for small-scale legibility" |
| `icon-trophy-record-quantum-gold` | Stern-Medaillon sitzt IN der Schale — bei 12–14px (Mobile-Nav, Stats-Records) von `trophy-tournament`/`trophy-win` kaum zu unterscheiden, da nur ein Interieur-Detail variiert, nicht die Außensilhouette. | „a compact trophy cup topped with a single five-pointed star medallion insert set into its bowl, symbolizing a personal best record achievement" | „a compact trophy cup with a single large five-pointed star mounted as a raised finial spike above the rim instead of inside the bowl, star silhouette clearly breaking the cup's top outline, symbolizing a personal best record achievement, bold simple silhouette for small-scale legibility" |

**Nach Freigabe:** `icon-consolidation-batch-01.manifest.json` mit den Neu-Prompts aktualisieren, L0-Status in §1 auf „🟡 Regeneriert (2026-09-XX)" ändern, dann `npm run design:generate --dry-run` → nach Sichtprüfung `--yes`.

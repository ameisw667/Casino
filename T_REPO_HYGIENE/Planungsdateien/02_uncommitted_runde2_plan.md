...
> **Status:** 🟢 Executed (2026-09-17) — K10–K16 committet, K17 bleibt Jan · **Stand:** 2026-09-17 · **Owner:** LLM (Jan nur bei Gate) · **Scope:** Inventarisierung und sequenzielle Commit-Orchestrierung des nach Runde 1 (`01_uncommitted_cohort_review_plan.md`) verbliebenen unveröffentlichten Arbeitsstands auf `codex/uncommitted-cohort-review` — keine neue Feature-Arbeit, kein Push, kein Merge, keine visuelle Selbstbewertung.
...
| ------ | ------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------- | ------------ | -------------- | --------------------------------------------------------------------- |
| K10    | Store-Test-Split (03a-R09) committen                                     | 1 gelöscht + 6 neue Testmodule + `helpers/` + `vitest.config.ts` + `xx_docs/07_state_store_context.md` + `docs/frontend/03_state_management_persistence.md`                  | Sequenziell            | 🟢 Umgesetzt | LLM            | 78/78 Testblöcke erhalten; `npm test`                                 |
| K11    | Design-Assets-Pipeline-Ausbau committen                                  | 4 neue Module + 4 neue Tests + `style-preset`/`lifecycle`/`client` + 6 Scripts + `asset-index.json` + `review-gallery.html` + `xx_sop/21` + `T_IMAGE_CREATION/**` (10)       | Sequenziell            | 🟢 Umgesetzt | LLM            | `npm test -- src/lib/design-assets` + typecheck                       |
| K12    | Tooling-Gates committen (03a-R07/R10)                                    | `.github/workflows/quality-ci.yml`, `package.json`, `scripts/check-duplication.mjs`, `scripts/check-file-sizes.mjs`                                                          | Sequenziell            | 🟢 Umgesetzt | LLM            | `npm run check-file-sizes` Exit 0 (belegt); `check-duplication` läuft |
| K13    | Claude-Code-Dokupaket committen (`t_claude_code/**` + Security-Übergabe) | ~50 neue + ~20 geänderte Doku-Dateien; davon `T_SECURITY_HARDENING/11_status_quo_and_next_actions_prompt.md` als Ziel-Link von K14                                           | Sequenziell            | 🟢 Umgesetzt | LLM            | `npm run check-doc-links` (Live-Anteil)                               |
| K14    | **CI-Blocker: 5 tote Live-Links auflösen**                               | `worldmap/05_ZUKUNFTSPLANUNG.md` (3 Links), `T_FRONTEND/00_AUFGABEN_FRONTEND.md` + `T_FRONTEND/Startseite/03_Informationshierarchie/Plan.md` (2 Links), ggf. `.gitignore:99` | Sequenziell (nach K13) | 🟢 Umgesetzt | LLM + Jan-Gate | Clean-Worktree-Lauf von `check-doc-links` → Exit 0                    |
| K15    | Frontend Testing-v3 / Hero-Scrolly committen                             | 3 Komponenten, `scripts/capture-testing-v3-audit.mjs`, `T_FRONTEND/01_spielfunktion.md`, `T_FRONTEND/Planungsdateien/06_testing_v3_werbevideo_scrolly_plan.md`               | Sequenziell            | 🟢 Umgesetzt | LLM            | typecheck + lint; Sichtprüfung = Jan                                  |
| K16    | Money-Pfad: Wallet-Domänenmarker + Guide-Fassade                         | `src/lib/casino/wallet.ts` (nur Kommentare), `xx_sop/06_service_layer_casino.md`, `src/lib/casino/guide-knowledge/index.ts` (neu)                                            | Sequenziell (nach K12) | 🟢 Umgesetzt | LLM            | `security-reviewer` + typecheck; Nachweis „keine Logikänderung"       |
...
## 6 — Execution-Log (2026-09-17, Branch `codex/uncommitted-cohort-review`)

Alle Kohorten K10–K16 sind als **je ein Commit** auf dem Arbeitsbranch gelandet — keine Kohorte wurde aufgesplittet, kein `git add .`/`-A` (ausschließlich explizite Pfade), kein `--no-verify` (Hook-Kette aus Migrations-Kollisions-Check + `gitleaks protect --staged` + `lint-staged` lief bei jedem Commit scharf), kein Push, kein Merge, kein Rebase, keine `--force`-Aktion.

| Kohorte | Commit     | Inhalt                                                                  | Gate-Nachweis                                                                                                                                 |
| ------- | ---------- | ----------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| K10     | `d4d5a8b5` | Store-Test-Split (03a-R09) als Einheit                                  | 78/78 Testblöcke erhalten; Suite grün                                                                                                         |
| K11     | `f6f4678d` | Design-Assets-Pipeline-Ausbau                                           | `npm test` + typecheck; `lint-staged` formatiert nach                                                                                         |
| K12     | `517c1566` | Tooling-Gates 03a-R07/R10 (Warn-Gate Duplikation + Hartgate Dateigröße) | `check-file-sizes` Exit 0 (1.043 Dateien, keine neuen Überschreiter)                                                                          |
| K13     | `d9caf069` | Claude-Code-Dokupaket + Security-Übergabe                               | `check-doc-links` (Live-Anteil)                                                                                                               |
| K14     | `3f3660cb` | G5 Option A: `xx_sop/shared/` versioniert, 5 tote Live-Links gelöst     | **Clean-Worktree-Lauf `check-doc-links` → Exit 0, „0 tote Links in lebenden Dateien"**                                                        |
| K15     | `d8d40842` | Testing-v3-Sandbox & Hero-Scrolly-Polishing                             | typecheck + lint; Sichtprüfung offen = Jan                                                                                                    |
| K16     | `d57c8056` | Wallet-Domänenmarker (Money-Pfad) + `guide-knowledge`-Fassade           | `security-reviewer` → **PASS** (kein CRITICAL/HIGH/MEDIUM); `git diff` wallet.ts: 15 Insertions, 0 Deletions, keine ausführbare Zeile berührt |

**Abschluss-Gates (Stand nach K16):** `npm test` 🟢 **262 Dateien / 1.862 Tests, Exit 0** · `npm run typecheck` Exit 0 · `npm run lint` 0 Errors (40 vorbestehende Warnings) · `check-file-sizes` Exit 0 · `check-duplication` report-only.

**Abweichungen vom Plan (transparent):**

1. **K10 und K11 wurden von einer parallel laufenden Session committet**, nicht von dieser. Bei K11 lag der Commit zeitlich zwischen meinem `git add` und meinem `git commit`, wodurch der Hook mit „lint-staged could not find any staged files" abbrach. Beide Cohorts wurden inhaltlich verifiziert (identische Dateimenge, Zeilendifferenzen nur aus `lint-staged`-Autofix) und als erledigt behandelt. Ab K12 wurde deswegen **Staging und Commit in einem einzigen Befehl** ausgeführt, um das Race-Fenster zu schließen.
2. **K14-Nebenwirkung:** Der Versionierung von `xx_sop/shared/` liegt `.gitignore:98–99` als auskommentierter Kommentarblock zugrunde (statt der gelöschten Ignore-Regel), damit die Entscheidung samt Umkehrweg im Repo nachvollziehbar bleibt. Umkehrbar über Regel zurück + `git rm --cached`.
3. **Eine LOW-Notiz aus dem Security-Review ist bewusst nicht gefixt:** die neue `guide-knowledge/index.ts`-Fassade erbt transitiv `import 'server-only'` plus den Service-Role-`createAdminClient` aus `pgvector-store.ts:1`. Verhalten ist fail-safe (Import in einer Client-Komponente bricht den Build), die Fassade dokumentiert die Vererbung aber nicht. Als Ergonomie-Punkt notiert, kein Blocker.

...
- **Task**: Holistic coordination. Resolves cross-agent conflicts. Owns roadmap alignment with `docs/CASINO_ROYALE_MARKET_ROADMAP.md`.
...
- **Files**: `package.json`, `next.config.*`, `src/app/layout.tsx`, `src/app/**/{page,layout}.tsx` (metadata)
- **Task**: 100% build stability and SEO hygiene.
- **Rules**: Every route must export `metadata` with title and description — in `page.tsx` for server components, or `layout.tsx` for client-component pages (client `page.tsx` cannot export `metadata`). Fix all `npm run build` warnings before merge. No unresolved dependency conflicts.

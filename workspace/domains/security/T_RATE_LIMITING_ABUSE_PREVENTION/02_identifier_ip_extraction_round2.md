...
- **Schritte:**
  ...
  5. Kollisionsbestätigungs-Test: zwei simulierte Requests mit identischem `User-Agent`/`Accept-Language` im selben 5-Minuten-Fenster erzeugen denselben Fallback-Bucket (Muster: bestehender IPv6-/64-Kollaps-Test `request-security.test.ts:180-194`, aber als _Kollisionsbeweis_ statt als Trennungsbeweis) — macht das dokumentierte MEDIUM-Risiko test-belegt.
- **Erwartetes Verhalten:** Jeder der 5 Tests dokumentiert das _tatsächliche_ aktuelle Verhalten (keine neuen Validierungen erzwungen, sofern nicht ausdrücklich als Bug identifiziert) — Ziel ist Sichtbarkeit und Regressionsschutz, nicht zwingend Verhaltensänderung.

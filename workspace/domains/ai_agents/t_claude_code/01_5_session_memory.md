...
### 3.2 — Latenz-Regel (Regeländerung → Memory-Update im selben Arbeitsschritt)

> Wird eine Verhaltens- oder Präferenzregel geändert (`t_claude_code/**`, `CLAUDE.md`, SOP), die durch einen bestehenden Memory-Eintrag gespiegelt wird, schlägt das LLM den zugehörigen Memory-Update **im selben Arbeitsschritt** vor — nicht „bei Gelegenheit in einer späteren Session"; die Freigabe bleibt Gate G1 (globale Regel „Write to memory only when I explicitly ask").

Primärer Ablageort der Regel: [`Planungsdateien/06_u07_pflegekadenz_plan.md`](Planungsdateien/06_u07_pflegekadenz_plan.md) §2b (mit dem belegten 8-Tage-Latenzfall `no-visual-check-frontend.md` als Begründung); Trigger-Seite („was löst Schreiben aus") dort §2a.

**Anker-Korrektur 2026-09-18 (U07-L2):** Der Abschnitt `§3.2` existierte in dieser Datei bisher **nicht** — verifiziert per Heading-Scan und `git log` (2026-09-18). Mehrere Dokumente verweisen dennoch auf ihn (`01_6_03_typ_abdeckung_project.md:20,65`, `01_6_07_pflegekadenz.md:16,36,44,59`, `01_multi_agent_wissensrueckfluss_plan.md:15,19`). Der Anker wird hier neu angelegt, damit diese Verweise auflösen. **Inhaltlich** meinten die Altverweise allerdings die Trigger-Definition mit zwei realen Testfällen (Migrations-Nummernkollision 049/050, falscher Testpfad) samt Guardrail-Begründung — diese steht tatsächlich in [`01_5_03_fehler_pattern_learning.md`](01_5_03_fehler_pattern_learning.md) §1, nicht hier. Die §3.1-Verweise (`01_multi_agent_wissensrueckfluss_plan.md:15`) bleiben offen und sind als Befund gemeldet, nicht stillschweigend umgeschrieben.

...
> **Status:** Verifiziert · **Stand:** 2026-09-17 · **Scope:** Ausschließlich Mobile-LCP für `/games/crash-multiplayer`. Keine Admin-, API-, Wallet- oder außerhalb des belegten Renderpfads liegende Produktverhaltensänderung.
...
## Finale Verifikation (2026-09-17)

Frischer Production-Build `xkCNVszxhUUASgCfpV5fz`, lokaler Production-Server, Pixel 5, 4× CPU-Drosselung und 4G (150 ms Latenz, 1,6 Mbit/s Downlink, 750 kbit/s Uplink); zwei Läufe ohne Interaktion. Alle Werte sind PerformanceObserver-Labwerte.
...
Belegte Ursache: Der servergerenderte Multiplikator-`H1` war beim DOMContentLoaded vorhanden, lag vor Hydration jedoch mit `x=635` außerhalb des Pixel-5-Viewports, weil Inline-Desktopwerte die bestehende Mobile-CSS überstimmten. Der erste neue Regressionstest auf Mobile-Overrides war erwartbar rot und nach der kleinsten Änderung grün (3/3). Nur die routenlokale `@media (max-width: 960px)`-Regel verwendet nun für Grid und zwei Ordnungswerte `!important`; damit bleibt Admin ebenso wie API-, Wallet- und Spiellogik unberührt. Die zuvor geprüften Backdrop- und Canvas-RAF-Hypothesen bleiben als unzureichend verworfen. Beide frischen PerformanceObserver-Lab-LCP-Werte liegen bei ≤ 2,5 s; `worldmap/60_MOBILE_LCP_ROUTE_STATUS.md` wurde deshalb aktualisiert.

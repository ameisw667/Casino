# 09 — Mobile-LCP: `/games/roulette`

> **Status:** Geplant · **Stand:** 2026-09-13 · **Scope:** Ausschließlich Mobile-LCP für `/games/roulette`. Keine Admin-, API-, Wallet- oder außerhalb des belegten Renderpfads liegende Produktverhaltensänderung.

## Route und dokumentierte Baseline

| Messwert               | PerformanceObserver-Lab-Baseline |
| ---------------------- | -------------------------------- |
| FCP Lauf 1 / 2         | 1,516 / 1,496 s                  |
| finaler LCP Lauf 1 / 2 | 5,604 / 5,444 s                  |
| Dominanter LCP-Befund  | Text nach mounted-/Bühnen-Gate   |
...
- Der finale LCP-Kandidat ist Text nach einem mounted-/Bühnen-Gate. Beide Läufe liegen deutlich über dem Ziel.
- Der dokumentierte dominante LCP-Befund bestimmt den Untersuchungsumfang dieser Route.

## Noch zu prüfende Hypothesen

- Das Gate koppelt die erste funktionsfähige Mobile-Textansicht an Canvas-, Motion- oder Client-Initialisierung.
...
- **Ursachenanalyse:** Die Verzögerung liegt im Zeitpfad zum initialen Text, nicht in einer dokumentierten großen LCP-Transferressource. Das mounted-Gate ist daher der primäre Untersuchungsgegenstand.
- **Minimaler Fixpfad:** Nur die kleinste statische, funktionale Mobile-Textgrundansicht vor dem belegten Gate planen; Roulette-Berechnung und Desktop-Bühne bleiben unverändert.
...
- Teilt den Bühnen-/Hydration-Pfad mit Crash-Multiplayer und Slots. Erst gemeinsame Ursache prüfen, aber jede Route separat freigeben.

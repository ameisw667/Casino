# 14 — Mobile-LCP: `/leaderboard`

> **Status:** Geplant · **Stand:** 2026-09-13 · **Scope:** Ausschließlich Mobile-LCP für `/leaderboard`. Keine Admin-, API-, Wallet- oder außerhalb des belegten Renderpfads liegende Produktverhaltensänderung.

## Route und dokumentierte Baseline

| Messwert               | PerformanceObserver-Lab-Baseline |
| ---------------------- | -------------------------------- |
| FCP Lauf 1 / 2         | 1,372 / 1,576 s                  |
| finaler LCP Lauf 1 / 2 | 2,784 / 3,224 s                  |
...
- Das Brand-Icon ist finaler LCP-Kandidat. Beide Läufe verfehlen das Ziel.
- Der dokumentierte dominante LCP-Befund bestimmt den Untersuchungsumfang dieser Route.

## Noch zu prüfende Hypothesen

- Der gemeinsame Header-/Icon-Pfad wird auf dieser Route verspätet entdeckt oder priorisiert.
...
- **Ursachenanalyse:** Der dokumentierte Kandidat ist kein Leaderboard-Daten- oder Tabelleninhalt. Daher liegt der erste Prüfpfad beim Brand-Icon, nicht bei der Ranglistenlogik.
- **Minimaler Fixpfad:** Nur eine minimale nachweisgestützte Verbesserung des Brand-Icon-Discovery-Pfads planen; Datenabfrage und Rankinglogik bleiben unverändert.
...
- Gemeinsamen Icon-Pfad mit /games, History, Sign-in, Sign-up und Stats prüfen; Messung bleibt route-spezifisch.

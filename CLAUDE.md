...
Functionality-only test route, deliberately outside the premium design system (no Design-Guardian rules apply here — it will never go live with real users). Above-the-fold, no scroll: header with email/password sign-in form (top-right), sidebar linking to every real page/game, main area showing session state. War der erste Baustein der Auth-Migration; die Haupt-App nutzt inzwischen dieselbe Supabase-Auth über `/sign-in`, `/sign-up` und `SupabaseSessionProvider` (siehe `02_CLERK_SUPABASE.md`).

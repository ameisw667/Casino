...
> **Update 2026-08-09 — Mobile-Perf Deep-Dive (Befund A–E) umgesetzt:** Siehe [docs/architecture/05_MOBILE_PERFORMANCE.md](../architecture/05_MOBILE_PERFORMANCE.md). Neue Lighthouse-Baseline gegen die neu gebaute Homepage: Mobile-Perf **72 → 75**, TBT **314 → 258 ms (−18 %)**, Desktop **90 / TBT 6 ms (keine Regression)**. Die alte 56/100-Zahl unten war gegen die Vorgänger-Homepage gefahren und ist durch diese Messung ersetzt. Offen: Befund E (LCP-dominiertes Hero-Bild, Doppel-Laden) für weitere LCP-Senkung.
>
> **Update 2026-08-09 (2. Deep-Dive, `worldmap/05_ZUKUNFTSPLANUNG.md` Initiative 1.1) — LCP-Root-Cause-Suche, Ziel < 2,5 s nicht erreicht:** Vollständiger Verlauf in [docs/status-reports/05_1.1_MOBILE_PERFORMANCE.md](05_1.1_MOBILE_PERFORMANCE.md) (M1–M8, 5 unabhängig verifizierte Fixes: Opacity-Mechanismus, `MainLayout`-Mount-Gate, WebGL-Bundle-Exclusion, Mouse-Listener, zusätzliches Chunk-Splitting). **Kein einziger davon veränderte den LCP messbar** (durchgehend 5,6–5,8 s in dieser Sandbox-Umgebung) — die Ursache liegt strukturell in der Grundlast (React/ReactDOM + Zustand-Store + Supabase-Client, ~370 KB) kombiniert mit dem CPU-Throttling-Profil dieser Messumgebung, nicht in einem einzelnen behebbaren Bug. Nicht abschließend geklärt, ob reale Endgeräte denselben Wert zeigen würden (siehe Empfehlung in der verlinkten Datei). Diese Sitzung ist unabhängig vom oben genannten „Befund E" (Hero-Bild-Doppel-Laden) — beide offenen Fäden sollten in einer möglichen Folge-Session zusammengeführt werden.

...
> **Verwandt, aber bewusst separater Scope:** Ein neues Job-Health-Panel (`GET /api/admin/job-health` + `AdminOverviewClient.tsx`) existiert seit 2026-09-02 auf `/admin` (nicht `/admin/analytics`) — Ergebnis der Nachbararbeit in [`T_BACKGROUND_JOBS_SCHEDULING/00_BACKGROUND_JOBS_SCHEDULING_UEBERSICHT.md`](../../workspace/domains/database/T_BACKGROUND_JOBS_SCHEDULING/00_BACKGROUND_JOBS_SCHEDULING_UEBERSICHT.md). Dieses Modul bleibt bei seinem ursprünglichen `/admin/analytics`-Scope, verweist hier nur darauf.

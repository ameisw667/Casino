...
1. **Browser-Mutation-Routen ohne Layer 2** (dynamischer Scan, 2026-09-12; **korrigiert 2026-09-13**): Ursprünglich 4 gemeldet (`auth/login-guard`, `auth/signup-suspicion`, `casino/migrate-session`, `casino/session-sync`). Das Runde-3-Security-Review korrigierte: `migrate-session` und `session-sync` sind konstante **410-Stubs** („… has been retired") — keine Mutation zu schützen, identische Story wie `webhooks/clerk`; sie sind inzwischen auf der Exemptions-Liste (Token `/retired/`). **Real verbleibende Lücken (2):** `auth/login-guard` (POST) und `auth/signup-suspicion` (POST) — auth-/session-bootstrap-bezogen, deshalb bewusst **nicht** im laufenden Scope angefasst (Verhalten-Risiko im Login-Flow). Sie sind im Inventar-Test als `OPEN_LAYER_2_GAPS`-Tripwire gepinnt: eine dritte Lücke oder ein Fix erzeugt bewusst einen Testfehler bis zur bewussten Listen-Aktualisierung. Kandidaten für Runde 3.

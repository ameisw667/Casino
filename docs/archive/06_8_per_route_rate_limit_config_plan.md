...
**Offene Fragen des Plans:** Q1 = (a) — nur die 4 unbenannten Money-Pfad-Werte konsolidiert, Admin-/Modul-Konstanten unangetastet. Q2 = (a) — interne plausible Herleitung, ehrlich als „unbelegter Erfahrungswert" gekennzeichnet, keine Außenrecherche.

| Meilenstein | Ergebnis                                                                                                                                                                                                                                                                                                                                                                     |
| :---------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| L0 (C1)     | ✅ Neue Datei `src/lib/security/rate-limit-config.ts` mit 8 benannten Konstanten (Werte 1:1 aus dem Code übernommen); 4 Routen (`bet`, `blackjack`, `bet-crash-multiplayer`, `redeem-code`) importieren sie statt der Roh-Literale. Scope-Strings unverändert → keine Counter-Reset-Risiken.                                                                                 |
| L1 (C7)     | ✅ Neue Tests `src/lib/security/__tests__/rate-limit-config.test.ts`: 8 Konstanten-Wert-Assertions (Pin auf Produktionswerte = negativer Kontrollfall) + 4 Integrationstests (je Scope (N+1)-ter Aufruf im Fenster abgelehnt, mit importierten Konstanten statt Testliteralen).                                                                                              |
| L2 (C5)     | ✅ Jede der 8 Konstanten trägt einen Begründungskommentar; Werte ohne echte Herleitung explizit als „unbelegter Erfahrungswert" markiert (Ehrlichkeitsgebot des Plans beachtet, keine Pseudo-Begründung erfunden).                                                                                                                                                           |
| L3 (C9)     | ✅ `xx_docs/08_api_backend_context.md`: bet 30/10s, bet-crash-multiplayer 30/10s, blackjack 20/10s, fraud/scan 1/300s — alle 4 Einheiten-Fehler korrigiert.                                                                                                                                                                                                                  |
| L4 (C10)    | ✅ Neue Tabelle §3.9 „Auth & Guards" mit `login-guard` (5/60s IP) und `signup-suspicion` (10/60s IP); Werte gegen `src/app/api/auth/**/route.ts` verifiziert.                                                                                                                                                                                                                |
| L5          | ✅ Verifikation: `npm run typecheck` 0 Fehler · `npm test` 206 Dateien / 1543 Tests grün · `npm run lint` 0 Errors (23 Warnungen, alle vorbestehend, keine in geänderten Dateien) · `npm run build` Exit 0 · `git status` nur geplante Dateien. Security-Review (security-reviewer): **PASS** — Werte bit-identisch, keine Import-Side-Effects, keine Scope-String-Änderung. |

**Niveau-Neubewertung nach Execution (Sub-Schnitt der 10 Sub-Unterkategorien):** C2/C6/C4/C8 unverändert (10/12/15/15) · C7 Top 40 → **Top 20** (Konstanten-Pin-Tests + (N+1)-Integrationstests) · C1 Top 55 → **Top 15** (alle 4 Money-Pfad-Werte benannt) · C5 Top 60 → **Top 25** (Herkunft dokumentiert, bleibt Erfahrungswert statt Lasttest) · C9/C10 Top 65/65 → **Top 15/15** (Doku-Drift behoben) · C3 unverändert **Top 70** (bewusst Eigentum von `06_6` L5, hier nicht doppelt). Neuer Schnitt: **Top 21,2 %** (gerundet Top 21 %) — Verbesserung von Top 41 % auf Top 21 %; Restschuld ist fast vollständig C3 (Doku-Drift Routenzahl/Telegram), der mit `06_6` L5 geschlossen wird.

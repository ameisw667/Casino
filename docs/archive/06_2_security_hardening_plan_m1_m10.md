...
- **Wichtiger Nebenbefund (nicht M10-Scope, nicht durch diese Session verursacht):** Während der M10-Verifikation tauchte in `npm run lint` plötzlich ein Sprung von 20 auf 987 Probleme (360 Fehler) auf — Ursache: ein bislang nicht vorhandener Ordner `worldmap/.research/tonejs/` (offenbar eine vollständig vendorte Kopie der Tone.js-Bibliothek samt Tests) ist während dieser Session auf der Festplatte erschienen, augenscheinlich **nicht** durch einen meiner Tool-Aufrufe (kein Bezug zu Audio/Tone.js in M1–M10). `git check-ignore` bestätigt: `worldmap/` ist über `.gitignore` Zeile 84 (`/worldmap/`) vollständig von Git ausgeschlossen — das gilt auch für **diese Plan-Datei selbst**, by design („planning-only docs … no product code"), nicht fehlerhaft. `eslint.config.mjs` ignoriert `worldmap/` jedoch **nicht** separat, weshalb die neu aufgetauchten Dateien plötzlich mitgelintet wurden. Keine meiner geänderten Dateien sind betroffen (per Grep verifiziert). Empfehlung an Jan: prüfen, was `worldmap/.research/tonejs/` erzeugt hat (evtl. eine parallel laufende Session) — nicht selbstständig gelöscht, da unklare Herkunft. **Weiterhin ungeklärt zum Zeitpunkt der Archivierung.**
...
- `worldmap/.research/tonejs/` (Nebenbefund aus der ersten Runde) weiterhin ungeklärt — nicht erneut geprüft, da außerhalb des Scopes und Jans eigene Prüfung aussteht.

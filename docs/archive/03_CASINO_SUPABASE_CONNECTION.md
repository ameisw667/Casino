...
| `009_meta_features.sql`                   | `user_identities`, `identity_link_quarantine`, `admin_roles`, Trigger `guard_canonical_user_provisioning`, `link_user_identity`                                              | ❌ **Nicht remote angewandt** (verifiziert)                       | Canonical-Identity-Layer + Admin-Rollen. _Frühere Doku behauptete fälschlich `community_goal`/`chat_messages`/`live_bets`._ `admin.ts` hat Fallback, Produktion läuft ohne 009. Aktivierung nur nach `worldmap/01a-db-rollout-plan.md` §3.2. |
...
| `012_welcome_bonus.sql`                   | `users.balance DEFAULT 10000.00` + 0-Balance-Update                                                                                                                          | ❌ **Nicht remote angewandt** (verifiziert)                       | Welcome-Bonus-Default. _Frühere Doku behauptete `promo_codes`/`promo_code_redemptions` — falsch. `promo_codes` wird von keinem Code queried (Phantom)._                                                                                      |

...
| ------ | -------------------------------------------------- | ------------------------- | ---------------------------------------------- | ------------- |
| L0     | Race-Condition-Tests für atomare Redis-Guards (T4) | 🟢 Verifiziert ausgeführt | — (siehe §8)                                   | LLM           |
| L1     | Red-Team-Skript-Unit-Tests (T5)                    | 🟢 Verifiziert ausgeführt | — (siehe §8)                                   | LLM           |
| L2     | CI-Coverage-Messung für Scope aktivieren (T7)      | 🟢 Verifiziert ausgeführt | — (siehe §8; Q2a: Gate beobachten, nicht hart) | LLM           |
| L3     | Guard-Stacking-Integrationstest (T10)              | 🟢 Verifiziert ausgeführt | — (Reihenfolge dokumentiert, siehe §8)         | LLM           |
| L4     | Signup-Guard-Edge-Case-Tests vertiefen (T2)        | 🟢 Verifiziert ausgeführt | — (siehe §8)                                   | LLM           |
| L5     | Testabdeckung + Doku-Nachzug (Abschluss)           | 🟢 Verifiziert ausgeführt | — (Vollsuite grün, Doku nachgezogen, siehe §8) | LLM           |
...
## 8 — Ausführungsprotokoll (2026-09-06, Status: verifiziert lokal)

**Alle Meilensteine L0–L5 ausgeführt.** 5-Stufen-DoD: `npm test` 1647/1647 grün (216 Dateien), `npm run typecheck` 0 Fehler, `npm run lint` grün, `npm run build` grün, git-Status-Audit nur geplante Dateien.

**L0 (Race-Tests):** Drei neue Parallel-Tests in `daily-cost-cap.test.ts`, `promo-guess-guard.test.ts`, `bet-velocity-guard.test.ts` (atomares Sync-Counter-Mock, `Promise.all` über Schwelle+5). Dokumentierte Grenze pro Plan: verifiziert Aufrufreihenfolge-Semantik, nicht echte Netzwerk-Nebenläufigkeit gegen einen Live-Redis.

**L1 (Red-Team-Unit-Tests):** Neue Datei `src/lib/security/__tests__/red-team-script-logic.test.ts` (16 Tests): echte Ausführung von `runRateLimitTarget`/`runAdminIdorProbe` (fetch gemockt: XFF-Rotation-Set Größe 10, Idempotenz-Key-Verteilung ⅔, 429-Contract, IDOR-Status-Contract), CLI-`main()`-Einträge (target-guard-Env-Gate, fehlender Cookie fail-fast, ephemeral-bootstrap mit gemockten Supabase-Clients + GITHUB_ENV-File-Verifizierung), `reportCliFailure`. Skripte dafür refraktoriert: Exporte + `fileURLToPath`-CLI-Guard (idiom aus `target-guard.ts`), gemeinsamer `reportCliFailure`-Export statt Inline-Arrow im `.catch` (hält Funktionsscope-Zahl klein für das globale 80 %-Schwellwert).
**Deviation dokumentiert:** Verifizierung per `npm test` (Vollsuite) statt `npx vitest run <Einzelpfade>` — stehende Projektregel (Antigravity Auto-Allow-Policy, keine variablen Dateipfade an Test-Runner).

**L2 (Coverage-Gate):** `vitest.config.ts` `coverage.include` um die 12 Scope-Dateien erweitert; per-file Schwellwerte = gemessener Ist-Wert (`coverage/coverage-summary.json`, 2026-09-06) minus Toleranz: request-security 94,5/91,1/100 → 92/89/98; signup-guard, daily-cost-cap, promo-guess, bet-velocity je 100/100/100 → 98/98/98; risk-signals 84,6/80/100 → 82/78/98; fraud-detection 90,3/66,7/100 → 88/64/98; network-fingerprint 100 → 98; responsible-gambling 95/90/100 → 93/88/98; login-guard route 100 → 98; target-guard 50/62,5/100 → 47/60/98; rate-limit-bypass 95,5/91,7/100 → 93/89/98; admin-idor 80/70/100 → 78/68/98; ephemeral-bootstrap 82,5/61,5/85,7 → 80/59/83. `.github/workflows/quality-ci.yml`: zusätzlicher, bewusst **nicht blockierender** Step `npm run test:coverage -- --coverage.reporter=json-summary` (`continue-on-error: true`) neben dem bestehenden `npm test`-Gate.
**Wichtiger Vitest-Befund (verifiziert in der Vitest-Source, `resolveThresholds`):** der globale `functions: 80`-Schwellwert gilt **additiv** auch für Dateien mit glob-Override („Global threshold is for all files, even if they are included by glob patterns") — Per-File-Overrides sind keine Ausnahme, sondern ADDITIONALE Checks. Die ungetesteten CLI-`.catch`-Arrows drückten admin-idor (66,7 %) und ephemeral-bootstrap (75 %) unter die globale Schwelle; behoben durch die `reportCliFailure`-Refraktorisierung + direkten Unit-Tests (alle 4 Red-Team-Skripte jetzt ≥ 80 % functions).
**Vorab-Fehler außerhalb des Scopes (dokumentiert, NICHT behoben — Kategorie-11-Scope):** `sentry-scrub.ts` functions 75 %, `useCasinoStore.ts` functions 57,1 %, `wallet.ts` branches 36,9 %/functions 90 %. Diese lassen `npm run test:coverage` mit Exit 1 enden — genau deshalb ist der CI-Step nicht blockierend (Q2-Empfehlung a: erst beobachten).

**L3 (Guard-Stacking):** T10-Lücke verifiziert und real. Neuer `describe` in `responsible-gambling-routes.test.ts` (statisches Assertion-Muster der Datei — kein Live-Integration-Harness vorhanden, im Datei-Kopf dokumentiert). Dokumentierte tatsächliche Reihenfolge im bet-Route: Origin → Auth → Rate-Limit → **Wellbeing-Guard** (Self-Exclusion vor Loss-Limit, beide in `checkWellbeingGuard`) → Zod-Validierung → Settlement; Bet-Velocity ist **kein** blockierender Guard, sondern nur `after()`-Oberservability nach erfolgreichem Settlement (06_1 L5) — mit allen drei aktiv erhält der Nutzer genau **einen** eindeutigen Fehlercode (SELF_EXCLUDED, 403), und es wird kein Fraud-Signal für ohnehin abgelehnte Requests geschrieben. Warum richtig: Validierungs-Fehler dürfen vor der Wellbeing-Entscheidung nicht an gesperrte Nutzer leaken, und der Limiter muss Last abbauen, bevor die Guard-DB-Query läuft (06_2 Security-Review). Keine Produktionscode-Änderung.

**L4 (Signup-Guard-Edge-Cases):** NaN (Zeile 25) und Future-Timestamp (Zeile 26) waren bereits gedeckt; ergänzt: `formRenderedAtMs: -1` (Pre-Epoch → elapsed riesig, kein Timing-Hit) und runtime-`undefined` (elapsed → NaN) — beide fail-open `null`.

**L5 (DoD + Doku):** Zahlen oben. Kategorie-Datei und Overview nachgezogen (siehe jeweilige Dateien); dieser Plan wird nach Protokoll per `git mv` nach `docs/archive/` verlagert.

**Offene Fragen Q1/Q2 — mit Empfehlung umgesetzt (Jans Freigabe, 2026-09-08):** Q1 = (a) kein Mutation-Testing-Meilenstein (YAGNI, keine neue Dependency). Q2 = (a) Coverage-Gate erst beobachten, nicht blockierend — der `continue-on-error`-Step läuft ab jetzt in jedem quality-ci-Lauf und liefert die Beobachtungsdaten.

**Cloud-Isolation:** Diese Ausführung berührte ausschließlich Unit-Tests, statische Assertions und Konfiguration — keine Probe-Läufe, keine Cloud-Kontaktpunkte (`.env.local`, Cloud-Projekt `hmqwozhdckbwjqzcmire`, Upstash-Instanz unberührt).

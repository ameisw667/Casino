...
> **Status:** Executed (archiviert) · **Stand:** 2026-09-05 (Execution 2026-09-06) · **Owner:** LLM · **Scope:** Unterkategorie #4 aus [`06_rate_limiting_abuse_prevention.md`](06_rate_limiting_abuse_prevention.md) (Top 18 %). Alle Zuständigkeiten liegen beim LLM; kein Meilenstein ist auf eine Jan-Entscheidung während der Ausführung angewiesen. Diese Datei ist so geschrieben, dass eine komplett neue, kontextlose LLM-Konversation sie ohne Rückfragen ausführen kann. Ausführungsprotokoll: Abschnitt 8.
...
| :------------------------------------ | :----- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| L0 Alert-Verdrahtung (F5)             | 🟢     | `recordRiskEventBestEffort()` ruft nach erfolgreichem `record_risk_event`-RPC bei `severity: 'high'` UND `occurrences === 1` UND vorhandener Event-ID `fraudAlertWait.trigger()` fire-and-forget auf (`src/lib/casino/risk-event-store.ts`, `triggerFraudAlertBestEffort()`). Score-Feld (Schema-Pflicht, Risk-Events tragen keinen numerischen ML-Score) wird mit dem Erstvorkommens-Zähler gefüllt; Kontext in `details`. 6 Unit-Tests (`risk-event-store.test.ts`): Erstvorkommen triggert, `occurrences > 1` nicht, low/medium nie, fehlende ID/RPC-Fehler nicht, Trigger-Fehler bricht Recording nicht.                                                                                                                                                                                                                                                                                         |
| L1 `balance_correction`-Producer (F6) | 🟢     | Q1(a) umgesetzt: `src/app/api/admin/users/route.ts` PATCH emittiert nach erfolgreicher `admin_update_user`-RPC (nur bei geändertem `balance`) ein `balance_correction`-Signal (severity `low`, Evidence: `changedFields`/`transactionId`/`replayed`) via `after()` — nicht-blockierend. Use-Case-Klärung: Admin-Balance-Änderung (`UsersPageClient.tsx` → `admin/users` PATCH) war die einzige Balance-Änderungsart ohne Signal-Spur; Annahme bestätigt. 2 Route-Tests (`admin-users-balance-correction.test.ts`).                                                                                                                                                                                                                                                                                                                                                                                   |
| L2 Eskalation nach 48h (F4)           | 🟢     | Q3(a): genau eine Eskalationsrunde. Timeout-Zweig von `executeFraudAlertWait()` triggert `fraudAlertWait` erneut mit `escalated: true` und verschärfter Nachricht („🚨 ESKALATION …"); eskalierte Läufe eskalieren nicht erneut. Eskalations-Run erhält eigenen Wait-Token-Idempotency-Key (`-escalation`-Suffix), sonst kollidierte er mit dem bereits abgelaufenen Token des Original-Runs. `maxDuration` um 1h-Puffer erhöht (172800+3600, s. u.). 5 neue Tests in `fraud-alert-wait.test.ts`.                                                                                                                                                                                                                                                                                                                                                                                                    |
| L3 Reopen-Status (F9)                 | 🟢     | Migration [`supabase/migrations/065_risk_event_reopened_status.sql`](../supabase/migrations/065_risk_event_reopened_status.sql): CHECK um `reopened` erweitert (NOT VALID/VALIDATE, 030/040/062-Muster), `review_risk_event()` um `reopen`-Aktion (nur aus `closed`/`reviewed`, nie aus `open`) ergänzt, Grants idempotent neu ausgestellt. Transport-Schicht: `admin/fraud`-Route akzeptiert `reopened` (Liste + Review), Skip der Wait-Token-Completion bei Reopen; `FraudPageClient.tsx`: Status-Badge/-Filter + „Wieder öffnen"-Aktion für `closed`/`reviewed`. **Bewusste Abweichung vom Plan (Abschnitt 3):** Plan empfahl Umsetzung in derselben Migration wie `06_3`s `suppressed`-Status; da Jans vorgegebene Ausführungsreihenfolge #4 vor #10 läuft, wurde eine separate Migration 065 gewählt — `06_3` folgt mit einer eigenen Migration, in beiden Protokollen dokumentiert.            |
| L4 ML-Kalibrierungs-Doku (F8)         | 🟢     | Kommentar über `SEVERITY_HIGH/MEDIUM/LOW` in `fraud-ml/scan.ts` benennt den „Literatur-Konvention, nicht projektspezifisch kalibriert"-Status ehrlich (konsistent mit `fraud-detection.ts:8-9`). Keine Nachkalibrierung (Plan-Nicht-Scope).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| L5 RPC-Integrationstest (F10)         | 🟢     | Neuer pgTAP-Test [`supabase/tests/risk_events_rpc.test.sql`](../supabase/tests/risk_events_rpc.test.sql) (20 Subtests) gegen die lokale Supabase-Instanz (`npx supabase test db`): Lock-Erwerb nach Release, Ablehnung bei frischer Sperre, Staleness-Fallback nach simuliertem Ablauf, `occurrences`-Upsert-Verhalten (Erstvorkommen/Replay, kein zweites Event), Status-Übergänge inkl. `closed → reopened` ✅ / `open → reopened` abgewiesen. Ergebnis: **PASS** (47 Tests über 5 Files, 0 Failures). Das Repo hatte zuvor keinen Live-Postgres-Test für diese RPCs — das etablierte Muster ist seit Migration 064 pgTAP (`npx supabase test db`), dem dieser Test folgt (statt des im Plan genannten TS-Dateinamens `admin-fraud-rpc-integration.test.ts`, für den kein Muster existierte). Migration 065 wurde dabei via `supabase migration up` auf die lokale DB angewendet und läuft sauber. |
| L6 Abschluss                          | 🟢     | Typecheck exit 0 · `npm run test` **1567/1567 grün** (208 Files) · `npm run lint` 0 Errors (23 vorbestehende Warnings) · `npm run build` exit 0 · pgTAP PASS (oben). Kategorie-Doku aktualisiert (Kategorie-Datei, Overview, T_ Übersicht).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |

### 8.2 Security-Reviews (Freigabe-Gates)

- **security-reviewer** (L0/L1/L2 + Route-Änderungen): **kein CRITICAL/HIGH.** Drei MEDIUM/LOW-Befunde getrigert:
  - MEDIUM (Reliability, behoben): `maxDuration` war exakt 172800s = Token-Timeout — der Run konnte im selben Moment gecancelt werden, in dem der Token-Timout auflöst, und die Eskalation hätte nie gefeuert. Fix: 1h-Puffer (`172800 + 3600`) mit Begründungskommentar. Verifikation gegen Trigger.dev-4-Semantik (ob Wartezeit in `maxDuration` zählt) bleibt ein Live-Deploy-Check, dokumentiert.
  - MEDIUM (vorbestehend, außerhalb des 06_9-Scopes, dokumentiert): Client-geliefertes `tokenId` in `admin/fraud` PATCH wird nicht gegen das Event verifiziert — ein Admin-Request könnte einen fremden Wait-Token abschließen. Befund existierte vor diesem Plan (der Route-Code wurde nur um `reopened` erweitert); Remediation (Token-ID serverseitig am Event speichern) ist ein eigener Folgeauftrag, nicht stille akzeptiert.
  - LOW (dokumentiert): Fire-and-forget `.trigger()` kann in serverloser Umgebung nach Response-Ende verloren gehen (bewusste Abweichung vom `after()`-Muster: Hook liegt in einer Lib, die auch direkt von Tests aufgerufen wird; Alerting ist Observability, kein Money-Pfad).
- **@migration-security-guard** (065): **PASS** — alle 5 Regeln nicht ausgelöst; TOCTOU-Fenster der Transition-Prüfung als begrenztes, admin-only Race dokumentiert; Constraint-Swap folgt dem 030/040/062-Sperrmuster.

### 8.3 Entscheidungen der Ausführungssession (Empfehlungs-Optionen)

- Q1(a): Producer gebaut (siehe L1). · Q2(a): kein Vier-Augen-Meilenstein. · Q3(a): genau eine Eskalationsrunde.

### 8.4 Niveau-Neubewertung der 10 Sub-Unterkategorien (Sub-Schnitt)

| #   | Sub-Unterkategorie              | Vor      | Nach     | Begründung                                                                                  |
| :-- | :------------------------------ | :------- | :------- | :------------------------------------------------------------------------------------------ |
| F1  | Advisory-Lock-Selbstheilung     | Top 12 % | Top 12 % | unverändert (pgTAP bestätigt bestehendes Verhalten)                                         |
| F2  | Review-Audit-Trail              | Top 12 % | Top 12 % | unverändert                                                                                 |
| F3  | Concurrent-Scan-Rejection       | Top 15 % | Top 15 % | unverändert                                                                                 |
| F10 | RPC-Logik nur gemockt           | Top 40 % | Top 12 % | echter Live-Postgres-Test für alle drei RPCs (pgTAP)                                        |
| F9  | Kein Reopen                     | Top 45 % | Top 12 % | `reopened`-Status mit Transition-Guard, Route + UI, pgTAP-verifiziert                       |
| F4  | Keine Eskalation nach 48h       | Top 35 % | Top 15 % | genau eine Eskalationsrunde; Rest: best-effort-Delivery, Live-Verifikation offen            |
| F7  | Keine Vier-Augen-Regel          | Top 55 % | Top 55 % | Q2(a) bewusst nicht umgesetzt (Prozess-Overhead im Lernprojekt)                             |
| F8  | Fehlende Kalibrierungs-Doku     | Top 60 % | Top 12 % | Ehrlichkeits-Doku nachgetragen (Kalibrierung selbst bleibt Folgeaufgabe)                    |
| F6  | Verwaistes `balance_correction` | Top 80 % | Top 12 % | Producer gebaut, getestet, nicht-blockierend                                                |
| F5  | Alert-Pfad nie verdrahtet       | Top 90 % | Top 15 % | verdrahtet, Erstvorkommen-only, getestet; Rest: serverless-Delivery-Risiko + tokenId-Befund |

**Neuer Sub-Schnitt:** (12+12+15+12+12+15+55+12+12+15)/10 = **Top 17,2 %** (gerundet **Top 17 %**, 🟡). Ausgangslage: Top 44,4 %. Kategorienniveau #4: Top 18 % → **Top 17 %**.

### 8.5 Verbleibende offene Punkte (bewusst dokumentiert, nicht still)

1. tokenId-Binding in `admin/fraud` PATCH (vorbestehendes MEDIUM) — eigener Folgeauftrag.
2. Eskalation/Alert-Delivery gegen echtes Trigger.dev-Live-Verhalten verifizieren (maxDuration-Puffer-Annahme).
3. ML-Schwellenwerte bleiben unkalibriert (nur ehrlich dokumentiert, Plan-Nicht-Scope).
4. Vier-Augen-Prinzip abgelehnt (Q2a) — bewusste Design-Entscheidung.

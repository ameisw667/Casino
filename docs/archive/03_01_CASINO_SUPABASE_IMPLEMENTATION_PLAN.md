# 03-01 — World Map: Supabase-Implementierungsplan & Execution Roadmap

> **Erstellt:** 2026-08-09 · **Status:** Ausführung · **Ziel:** Vollumfängliche Server-Autorität in Supabase ohne lokale Speicher-Abhängigkeit.

---

## 1 — Bereinigung von Dateiduplikaten

- **Aktion**: Entfernung der duplizierten Groß-/Kleinschreibungs-Variante `03_Casino_Superbase_Connection.md`.
- **Status**: ✅ Erfolgreich gelöscht. Nur noch `03_CASINO_SUPABASE_CONNECTION.md` existiert als kanonische Referenz.

---

## 2 — SQL-Migration 016: Complete Server-Authority Expansion

Das folgende Skript wird in `supabase/migrations/016_full_server_authority_expansion.sql` abgelegt. Es stellt alle bisher im Client vorgehaltenen Datenstrukturen auf atomare Supabase-RPCs um.
...
## 3 — Umsetzungs-Roadmap im Code (Next.js & Zustand)

### API Endpoints

1. `src/app/api/chat/route.ts` — `GET` (Chatverlauf laden) & `POST` (Nachricht senden via Supabase RPC).
2. `src/app/api/casino/seeds/route.ts` — `GET` (Seed-Status laden) & `POST` (Client-Seed ändern & Server-Seed rotieren).
3. `src/app/api/community/route.ts` — `GET` (Echte Einsatzsumme aus Supabase abfragen).
4. `src/app/api/casino/active-round/route.ts` — `GET` (Laufende Spielrunden für Re-Hydrierung prüfen).

### Service-Layer (`src/lib/casino/wallet.ts`)

- Ergänzung um statische Methoden:
  - `WalletService.getChatMessages()`
  - `WalletService.postChatMessage()`
  - `WalletService.getUserSeeds()`
  - `WalletService.rotateUserSeed()`
  - `WalletService.getCommunityStats()`
  - `WalletService.getActiveRound()`

### Store-Layer (`src/store/useCasinoStore.ts`)

- Entfernen der Dev-Methode `syncToFile()`.
- Erweiterung von `initialize()`: Abrufen von Wallet, Stats, Achievements, Seeds, Community-Ziel & Chat aus Supabase.
- Ausschluss verbleibender temporärer Daten aus dem LocalStorage Partialize-Filter.

---

## 4 — Verifikation & Qualitätskontrolle

1. **SQL Syntax & Execution Check**: Einspielen von Migration 016.
2. **Strict TypeScript Compliance**: `npx tsc --noEmit`.
3. **Vibe Check Verification**: `npm run vibe-check`.
4. **Automated Unit Tests**: `npm run test`.

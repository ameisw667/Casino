...
- **Scope:**
  - Button _„Ohne Passwort anmelden“_ im Sign-In Formular.
  - Ansicht 1: E-Mail eingeben $
ightarrow$ Klick auf _„Einmal-Code & Link senden“_ ruft `supabase.auth.signInWithOtp({ email, options: { redirectTo } })` auf.
...
ightarrow$ Eingabe des Codes ruft `supabase.auth.verifyOtp({ email, token, type: 'email' })` auf.
  ...
  - 60s Re-Send-Timer und Zurück-Button _„Mit Passwort anmelden“_.
...
- **Scope:**
  ...
  5. `docs/auth/13_master_summary.md` (Level 8 auf 🟢 Executed setzen).

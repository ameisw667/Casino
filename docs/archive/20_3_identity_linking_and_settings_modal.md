...
| **Modal-Struktur**       | **3 Tabs in `SettingsModal.tsx` (740px × 480px)**                            | 1. _Audio & Anzeige_ (Volume, Balance verbergen)<br>2. _Sicherheit & Login_ (Passkeys, 2FA, Konten verknüpfen)<br>3. _Benachrichtigungen_ (Telegram-Bot)                                    |
...
- **Scope:**
  1. `src/components/casino/LinkedAccountsSection.tsx`:
     ...
     - Button _„Google verknüpfen“_ ruft `supabase.auth.linkIdentity({ provider: 'google', options: { redirectTo: window.location.href } })` auf.
  ...
  2. `src/components/casino/SettingsModal.tsx`:
     ...
     - Linke Navigationsleiste mit 3 Tabs (_Audio & Anzeige_, _Sicherheit & Login_, _Benachrichtigungen_).

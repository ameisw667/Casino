...
export class GameBetController {
  ...
  static validateBet(req: BetRequest): BetValidationResult {
    if (req.amount < 0.1) return { valid: false, error: 'Minimum bet: $0.10' };
...
export class GameResultHandler {
  ...
  private static showNotification(event: GameResultEvent): void {
    ...
    const message = event.win ? `Won $${event.payout.toFixed(2)}!` : 'Bet lost';
...
**Loss of Capability**: None — actually _adds_ refund logic on error.
...
**Loss of Capability**: None — actually _improves_ FPS consistency.

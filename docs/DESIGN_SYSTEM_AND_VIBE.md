...
> **Kanonische Design-Quelle** des Casino-Projekts (Single-Source-of-Truth). Der `CLAUDE.md`-Abschnitt „Design System Rules" ist die kondensierte Kurzform; dieses Dokument ist die detaillierte, vollständige Referenz. Bei Widerspruch gilt diese Datei als maßgeblich.
...
1. **UI & Text (Modern Sans-Serif):**
   - _Einsatz:_ Für Navigation, Buttons, Fließtext, Beschreibungen.
   - _Charakter:_ Sauber, modern, hochgradig lesbar — geladen via `next/font` als `IBM Plex Sans` (Variable `--font-inter`); Display-Alias `--font-outfit` → `var(--font-inter)` (Ein-Familien-Terminal-System).
2. **Werte & Tabellen (Technical / Mono):**
   - _Einsatz:_ **EXKLUSIV** für Kontostände, Multiplikatoren (z.B. `1.45x`), Leaderboards, Game-Historie und Wettbeträge.
   - _Charakter:_ Monospaced (jeder Buchstabe ist gleich breit) — geladen via `next/font` als `IBM Plex Mono` (Variable `--font-mono`), `tabular-nums` für stabile Zahlenausrichtung. Das verhindert, dass das Layout bei schnellen Live-Updates (z.B. der hochzählende Multiplikator bei Crash) wackelt oder flackert.
...
- **Physik:** Wir nutzen standardmäßig **Spring-Physik** (Federn) statt linearen Übergängen.
  - _Beispiel:_ Wenn ein Modal öffnet, skaliert es nicht stur von 0 auf 100%, sondern schießt kurz auf 105% und federt sanft auf 100% zurück.
  - _Code-Beispiel:_ `transition={{ type: "spring", bounce: 0.4, duration: 0.6 }}`

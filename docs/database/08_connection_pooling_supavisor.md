...
> **Säule:** 8 von 10 · **Status:** 🟢 Doku-Qualität hoch / System-Reifegrad **Top 35 %** (Worldmap-Kategorie 02, Unterkategorie 10) — siehe Hinweis unten · **Stand:** 2026-09-13 · **Owner:** Jan / LLM  
> **Worldmap-Zuordnung:** Kategorie 02 (Unterkategorie 10: Connection-Handling & Skalierung / Pooling — Niveau: **Top 35 % · 🟢**)

> ⚠️ **Status-Präzisierung (2026-09-13, Säule 8 L1):** Doku-Qualität ≠ System-Reifegrad. Die vorherige Behauptung „Top 1 % — Weltklasse" beschrieb die Dokumentationsqualität, nicht den verifizierten Betriebszustand: Das hier beschriebene Health-Check-Skript existierte bis zur Ausführung von `T_DATABASE/08_database_connection_pooling.md` L2 nicht als Datei, die Schwellenwerte in §5 waren nie empirisch verifiziert, und im Bet-Pfad gab es keine Retry-Logik. Reifegrad-Befund und Messung: siehe Planungsdatei `T_DATABASE/08` Abschnitt 2.
...
## 3 — Die Konfiguration in `supabase/config.toml` (lokal) vs. Remote (Supabase-Dashboard)

Es gelten **zwei getrennte, bewusst unterschiedliche Konfigurationsebenen**:

**Lokal (Repo-Datei `supabase/config.toml`, verifiziert 2026-09-13, Zeilen 44–54):**
...
**Remote-Produktion (Nano-Tier, nur im Supabase-Dashboard sichtbar, nicht in einer Repo-Datei):**
...
**Warum die Werte abweichen (Annahme, nicht verifizierte Tatsache):** Lokal ist die Konfiguration großzügiger dimensioniert (`default_pool_size` 20 statt 15), damit lokale Testläufe (pgTAP-Suite, Lasttest, Konkurrenz-Tests) nicht an der Poolgröße scheitern; Remote ist der Wert vom Supabase-Nano-Tier vorgegeben. Falls Jan eine andere Begründung kennt, hier nachtragen. (Korrektur 2026-09-13, Säule 8 L1: Der Codeblock zeigte zuvor 15/200 irreführend als lokale `config.toml`-Werte — die Repo-Datei enthält tatsächlich 20/100.)
...
## 6 — Automatisiertes Monitoring-Skript (real implementiert, seit 2026-09-13)

Das Monitoring ist real implementiert als [`scripts/check-pooler-health.ts`](../../scripts/check-pooler-health.ts) (Säule 8 L2/N2; der frühere PowerShell-Codeblock war ein fiktives Doku-Beispiel ohne Datei im Repo):

```bash
# Lokal ausführen (K1, read-only gegen die lokale Instanz):
npm run db:pooler-health
```

- **Transport:** lokal (Standard) `npx supabase db query --local` mit einer reinen `SELECT`-Aggregation auf `pg_stat_activity` (gruppiert nach `application_name`/`state`); remote (nur wenn `SUPABASE_ACCESS_TOKEN` + `SUPABASE_PROJECT_REF` gesetzt sind, z. B. im L4-CI-Cron) via Management-API `/database/query/read-only` — dasselbe Muster wie `scripts/audit-query-performance.ts` (Säule 7 L7). Kein Schreibzugriff, kein `pg_terminate_backend` (das bleibt manuell und K4-gated, §7).
- **Ausgabe:** JSON auf stdout — `status` (`normal`|`warning`|`critical`), `activeConnections`, `threshold`, `breakdown`; Exit-Code 1 bei `warning`/`critical`.
- **Schwellenbewertung:** gegen die Werte aus §5 (Warnung ≥ 42 DB-Conns, Kritisch ≥ 55 DB-Conns).
- **Messgrenze:** `pg_stat_activity` sieht nur Server-seitige Verbindungen (Supavisor-Pool, PostgREST, …). Die Anzahl der Pooler-**Client**-Verbindungen (gegen `max_client_conn`) terminiert am Supavisor und ist aus SQL nicht ablesbar — sie wird im JSON als `poolerClients: null` gemeldet und wird durch den synthetischen Erschöpfungstest (`scripts/pool-exhaustion-test.ts`, Säule 8 N4) empirisch beobachtet.
- **Observability (N2):** wenn `SENTRY_DSN` gesetzt ist, wird das Ergebnis zusätzlich als Sentry-Event verschickt; ohne DSN bleibt das Skript unverändert bei stdout.

...
async function main() {
  ...
  const shelf = page.locator('div[aria-label="VIP Bookshelf Magazine Showcase"]').first();
  await shelf.waitFor({ state: 'visible', timeout: 10000 });

  const file1 = path.join(DOCS_DIR, '13_success_newsletter_bookshelf.png');
  const file2 = path.join(BRAIN_DIR, '13_success_newsletter_bookshelf.png');

  await shelf.screenshot({ path: file1 });
  fs.copyFileSync(file1, file2);
  console.log('SUCCESS: Captured 13_success_newsletter_bookshelf.png!');

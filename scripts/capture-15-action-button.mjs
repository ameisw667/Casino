...
async function main() {
  ...
  const button = page.locator('button[aria-label="ROLL DICE"]').first();
  await button.waitFor({ state: 'visible', timeout: 10000 });

  // Hover over the button to showcase felt texture and gold rim glow
  await button.hover();
  await page.waitForTimeout(400);

  const file1 = path.join(DOCS_DIR, '15_success_game_action_button.png');
  const file2 = path.join(BRAIN_DIR, '15_success_game_action_button.png');

  await button.screenshot({ path: file1 });
  fs.copyFileSync(file1, file2);
  console.log('SUCCESS: Captured 15_success_game_action_button.png!');

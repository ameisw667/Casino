...
async function main() {
  ...
  console.log('Waiting for compilation / mount...');
  const backdrop = page.locator('[data-testid="vault-prism-backdrop"]').first();
  await backdrop.waitFor({ state: 'attached', timeout: 20000 });
  await page.waitForTimeout(2500);

  const file1 = path.join(DOCS_DIR, '26_success_prism_gradient.png');
  const file2 = path.join(BRAIN_DIR, '26_success_prism_gradient.png');

  await page.screenshot({ path: file1 });
  fs.copyFileSync(file1, file2);
  console.log('SUCCESS: Captured 26_success_prism_gradient.png!');

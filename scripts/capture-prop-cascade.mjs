...
async function main() {
  ...
  const file2Docs = path.join(DOCS_DIR, 'prop_cascade_2_leaderboard.png');
  const file2Brain = path.join(BRAIN_DIR, 'prop_cascade_2_leaderboard.png');
  ...
  const file3Docs = path.join(DOCS_DIR, 'prop_cascade_3_dice.png');
  const file3Brain = path.join(BRAIN_DIR, 'prop_cascade_3_dice.png');
  ...
  await browser.close();
  console.log('Done capturing Letter Cascade proposals!');
}

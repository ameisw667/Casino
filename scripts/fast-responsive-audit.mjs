...
for (const pg of PAGES) {
          ...
          ({ vw, vh }) => {
            ...
            if (hOverflow > 1) {
              ...
              for (const el of document.querySelectorAll('body *')) {
                ...
                if (r.right > vw + 1 || r.left < -1) {
                  ...
                  culprits.push(
                    `<${el.tagName.toLowerCase()} class="${cls}"> (w:${Math.round(r.width)}px, r:${Math.round(r.right)}px)`,
                  );
                }
              }
              issues.push({
            ...
            for (const el of document.querySelectorAll('h1, h2, h3, p, span, button')) {
              ...
              if (el.scrollWidth > el.clientWidth + 2 && el.clientWidth > 0) {
                clipped.push(
                  `"${el.innerText.slice(0, 20)}" (scrollW:${el.scrollWidth}px > clientW:${el.clientWidth}px)`,
                );
              }
            }
            if (clipped.length > 0) {
            ...
            for (let i = 0; i < btns.length; i++) {
              for (let j = i + 1; j < btns.length; j++) {
                ...
                if (xO > 5 && yO > 5 && xO * yO > 100) {
                  overlaps.push(
                    `"${(btns[i].innerText || 'btn').slice(0, 15)}" & "${(btns[j].innerText || 'btn').slice(0, 15)}"`,
                  );
                }
              }
            }
  ...
  console.log(
    `Audited ${pg.name} (${pg.path}): ${flat.length === 0 ? '✅ PERFEKT (0 Issues)' : `⚠️ ${flat.length} Issues`}`,
  );
  for (const f of flat) {

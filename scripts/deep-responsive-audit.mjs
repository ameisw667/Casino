        ...
        if (hOverflow > 1) {
          ...
          for (const el of document.querySelectorAll('body *')) {
            ...
            if (r.right > vw + 1 || r.left < -1) {
              ...
              culprits.push(
                `${el.tagName.toLowerCase()}.${cls} (w:${Math.round(r.width)}px, right:${Math.round(r.right)}px)`,
              );
            }
          }
          issues.push({
          ...
          for (let j = i + 1; j < interactives.length; j++) {
            ...
            if (overlapArea > 0 && overlapArea / minArea > 0.35) {
              ...
              if (overlapDetails.length < 2) {
                const textA = (a.innerText || a.getAttribute('aria-label') || a.tagName)
                  .slice(0, 20)
                  .trim();
                const textB = (b.innerText || b.getAttribute('aria-label') || b.tagName)
                  .slice(0, 20)
                  .trim();
                overlapDetails.push(
                  `"${textA}" überlappt mit "${textB}" (Fläche: ${Math.round(overlapArea)}px²)`,
                );
              }
            }
          }

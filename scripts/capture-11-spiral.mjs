...
async function main() {
  ...
  const stage = page.locator('div[aria-label="Hall of Fame Spiral 3D Slider"]').first();
  await stage.waitFor({ state: 'visible', timeout: 10000 });

  const file1 = path.join(DOCS_DIR, '11_success_spiral_3d_slider.png');
  const file2 = path.join(BRAIN_DIR, '11_success_spiral_3d_slider.png');

  await stage.screenshot({ path: file1 });
  fs.copyFileSync(file1, file2);
  console.log('SUCCESS: Captured 11_success_spiral_3d_slider.png!');

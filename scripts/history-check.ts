...
const OUT_DIR = path.join(process.cwd(), 'history-screenshots');
...
async function run() {
  ...
  for (const vp of VIEWPORTS) {
    ...
    try {
      await page.goto('http://localhost:3000/history', { waitUntil: 'networkidle', timeout: 25000 });
      ...
      const checks = [
        ...
        { selector: 'text=REF', name: 'REF label in table' },
      ...
      // Verify reference tooltip contains full id
      const refCell = page.locator('td:has-text("REF")').first();
      const titleAttr = await refCell.locator('div[title^="Reference:"]').first().getAttribute('title').catch(() => null);
      if (!titleAttr || !titleAttr.includes('bet-crash-001')) {
        errors.push(`Reference tooltip not found or incomplete: ${titleAttr}`);
      }
  ...
  console.log('=== /history Headless Check Results ===');
